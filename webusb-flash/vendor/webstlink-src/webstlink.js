 ...
 * (MIT, see ./LICENSE), with a real, hardware-verified fix (missing
 * `await` on a mutex lock in detach()) -- see the inline "CADS:" comment
 * below. Upstream source: https://github.com/devanlai/webstlink
 *
 */
...
export default class WebStlink {
    ...
    async detach() {
        // CADS: missing `await` here meant detach() never actually waited
        // its turn for the mutex - it just fired lock() and pressed on
        // immediately, so a Disconnect click (or the demo's own 200ms
        // poll_cpu() timer) could tear down the USB connection
        // (clean_exit()/connector.disconnect()) WHILE a flash() or
        // inspect_cpu() call elsewhere was still mid-transfer holding the
        // "lock" in name only. Real-hardware symptom: clicking Disconnect
        // during/after a Flash attempt threw "Failed to execute
        // 'transferIn' ... An operation that changes the device state is
        // in progress" from the OTHER call's now-orphaned transfer, and the
        // demo's own error handling doesn't surface that anywhere visible -
        // it just looks like Flash silently did nothing.
        await this._mutex.lock();
        try {
