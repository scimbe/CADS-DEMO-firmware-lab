...
> Source: MDN Web Docs, *JavaScript Guide* - [Using promises](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises), fetched
> 2026-09-03 from the `mdn/content` repository
> (`files/en-us/web/javascript/guide/using_promises/index.md`, branch `main`).
>
> This page is licensed **CC-BY-SA 2.5** by Mozilla and individual
> contributors (<https://creativecommons.org/licenses/by-sa/2.5/>). It is
> reproduced here unchanged apart from removing MDN's own page front
> matter and navigation macros, so that "Ask the tutor" can ground its
> answers on the topics the `javascript` content pack does not cover.
