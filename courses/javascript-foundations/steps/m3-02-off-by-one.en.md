...
tasks:
  ...
  - id: read-the-symptom
    ...
    check: { type: question, prompt: { en: "Why is reading one element too far easier to find than reading one too few? Two sentences.", de: "Warum ist ein Element zu weit zu lesen leichter zu finden als eines zu wenig? Zwei Sätze." }, recallPrompt: { en: "For an array of length 3, `a[3]` returns `undefined` with no error, but `a[3].toString()` throws `TypeError: Cannot read properties of undefined`. Reading one element too few instead just produces a shorter result with no error at all. Why is the one-too-far mistake easier to notice than the one-too-few mistake?", de: "Bei einem Array der Länge 3 liefert `a[3]` `undefined` ohne Fehler, aber `a[3].toString()` wirft `TypeError: Cannot read properties of undefined`. Ein Element zu wenig zu lesen liefert dagegen nur ein kürzeres Ergebnis, ganz ohne Fehler. Warum fällt der Zu-weit-Fehler leichter auf als der Zu-wenig-Fehler?" }, rubric: "Separates a failure the runtime announces from one it does not: past the end the read yields a value with no properties, so the next access to it halts the program at once, while one element short yields a well-formed but shorter result that nothing objects to. Concludes that only a case sitting exactly on the boundary exposes the second. Does not pass: an answer in which the over-read throws at the read itself, or one that leaves the silence of the short result unexplained.", bloom: analyze, minChars: 60 }
socratic:
