...
misconceptions:
  ...
  - pattern: "Missing expected exception"
    question: { en: "The test waited for an error and none arrived. Does your catch block end without passing the failure on?", de: "Der Test wartete auf einen Fehler, und es kam keiner an. Endet dein catch-Block, ohne den Fehlschlag weiterzureichen?" }
    hints: [ { en: "A catch block that neither rethrows nor reports a failure hands back success for work that did not succeed.", de: "Ein catch-Block, der weder erneut wirft noch einen Fehlschlag meldet, gibt Erfolg für Arbeit zurück, die nicht gelang." }, { en: "Only one of the two functions here is allowed to absorb an error. The other has to let it travel on to its caller.", de: "Nur eine der beiden Funktionen hier darf einen Fehler aufnehmen. Die andere muss ihn zum Aufrufer weiterreisen lassen." }, { en: "Cleanup needs no catch at all: try/finally without catch runs the cleanup and leaves the error on its way.", de: "Aufräumen braucht gar kein catch: try/finally ohne catch führt die Aufräumarbeit aus und lässt den Fehler seinen Weg gehen." } ]
  - pattern: "cleanup"
