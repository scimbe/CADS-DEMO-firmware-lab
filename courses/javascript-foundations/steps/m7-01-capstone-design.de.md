...
tasks:
  - id: guess-pipeline
    ...
    check: { type: predict, prompt: { en: "Read examples/m7-pipeline.js. Write down all four values it prints before you run it.", de: "Lies examples/m7-pipeline.js. Schreib alle vier ausgegebenen Werte auf, bevor du es ausführst." }, then: { type: command, command: "node examples/m7-pipeline.js", seedMustFail: false, expectExitCode: 0, expectStdout: "guarded" }, recallPrompt: { en: "One line of a report carries an amount that is not a number, and the amounts are summed with `reduce` and then formatted with `toFixed(2)`. What does the total become, and does anything throw?", de: "Eine Zeile eines Berichts trägt einen Betrag, der keine Zahl ist; die Beträge werden mit `reduce` summiert und dann mit `toFixed(2)` formatiert. Was wird aus der Summe, und wirft irgendetwas?" }, rubric: "Stellt die vier vorhergesagten Werte den ausgegebenen gegenüber und nennt die früheste Stelle, an der die Verarbeitungskette entgleiste. Besteht nicht: die vier Werte wiedergeben, ohne zu benennen, wo der Schaden begann.", bloom: evaluate }
  - id: design-decisions
