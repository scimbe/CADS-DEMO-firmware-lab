...
tasks:
  ...
  - id: this-rules
    ...
    check: { type: question, prompt: { en: "The same function worked one way and failed the other. What decides its receiver? Two sentences.", de: "Dieselbe Funktion lief einmal und scheiterte einmal. Was bestimmt ihren Empfänger? Zwei Sätze." }, recallPrompt: { en: "Given `const t = { count: 0, tick() { this.count += 1; return this.count; } }`, calling `t.tick()` works, but pulling the method out first - `const fn = t.tick; fn();` - throws `TypeError: Cannot read properties of undefined (reading 'count')`. The function body did not change. What decides what `this` refers to at each of those two calls?", de: "Gegeben `const t = { count: 0, tick() { this.count += 1; return this.count; } }` funktioniert `t.tick()`, aber wenn man die Methode zuerst herauslöst - `const fn = t.tick; fn();` - wirft das `TypeError: Cannot read properties of undefined (reading 'count')`. Der Funktionsrumpf hat sich nicht geändert. Was entscheidet bei diesen zwei Aufrufen, worauf `this` zeigt?" }, rubric: "Schreibt den Empfänger der Aufrufstelle zu und nicht dem Ort, an dem die Funktion geschrieben wurde, erklärt, warum der scheiternde Aufruf keinen hat, und sagt, warum die Modulumgebung das meldet, statt einen Ersatz einzusetzen. Besteht nicht: der kürzeren Schreibweise die Schuld geben, oder eine Reparatur anbieten, ohne zu benennen, was den Empfänger liefert.", bloom: analyze, minChars: 60 }
socratic:
