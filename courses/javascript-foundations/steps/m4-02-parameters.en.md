...
tasks:
  ...
  - id: default-trigger
    ...
    check: { type: question, prompt: { en: "For which single argument value does a default fire, and what does joinWords(null, \\\"a\\\") return?", de: "Bei welchem einzigen Argumentwert greift ein Standardwert, und was liefert joinWords(null, \\\"a\\\")?" }, rubric: "The triggering value is the single one that stands for 'nothing was passed', and an omitted argument arrives as exactly it. The null call therefore does not fall back: null is taken as the separator, and since one word has no gap to fill, the result is that word alone. Does not pass: more than one triggering value, a result that only follows if the fallback had fired, or a result in which the separator appears despite there being nothing to separate.", bloom: analyze, minChars: 40 }
socratic:
