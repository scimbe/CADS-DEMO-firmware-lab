...
tasks:
  - id: guess-fallthrough
    ...
    check: { type: predict, prompt: { en: "examples/m2-switch-fallthrough.js calls price() three times with 'Apples', 'Cherries' and 'Mangoes'. Write down every line you expect, in order, and count them.", de: "examples/m2-switch-fallthrough.js ruft price() dreimal auf, mit 'Apples', 'Cherries' und 'Mangoes'. Schreib jede erwartete Zeile in der richtigen Reihenfolge auf und zähle sie." }, then: { type: command, command: "node examples/m2-switch-fallthrough.js", seedMustFail: false, expectExitCode: 0, expectStdout: "Bananas" }, recallPrompt: { en: "A `switch` case matches and its body has no `break`. What runs next, and where does it stop?", de: "Ein `switch`-Fall passt, und sein Rumpf hat kein `break`. Was läuft als Nächstes, und wo hört es auf?" }, rubric: "Stellt die vorhergesagte Zeilenzahl der ausgegebenen gegenüber und nennt den Aufruf, der mehr Zeilen lieferte als erwartet. Besteht nicht: eine nachträglich verteidigte Vorhersage von drei Zeilen, oder eine Antwort, die die Ausgabe wiedergibt, ohne den überraschenden Aufruf zu nennen.", bloom: evaluate }
  - id: grade
  ...
  - id: fallthrough-intent
    ...
    check: { type: question, prompt: { en: "Fall-through is a bug here and a feature two lines up. What does a reader see that tells them apart?", de: "Durchfallen ist hier ein Fehler und zwei Zeilen höher Absicht. Woran erkennt ein Leser den Unterschied?" }, rubric: "Nennt den sichtbaren Unterschied: gestapelte Marken ohne irgendetwas dazwischen gegenüber einem Rumpf, der Anweisungen ausführt und danach weiterläuft. Ergänzt, dass ein absichtlich weiterlaufender Rumpf einen Kommentar braucht, weil die Syntax ihn nicht kennzeichnet. Besteht nicht: eine Antwort, die nur sagt, break fehle, oder eine, die jedes Durchfallen für einen Fehler hält.", bloom: understand, minChars: 50 }
socratic:
