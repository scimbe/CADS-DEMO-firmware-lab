...
tasks:
  ...
  - id: why-silent
    ...
    check: { type: question, prompt: { en: "Neither bug threw. For each, name the single step where a wrong type slipped through.", de: "Keiner der Fehler warf. Nenne für jeden die eine Stelle, an der ein falscher Typ durchkam." }, rubric: "Locates two spots, one per defect, and each is an operator rather than a printed outcome. The first is an operator whose meaning is decided by the kinds it receives, so it silently switches job; the second is a comparison whose verdict was sealed before any data arrived, so nothing could ever satisfy it. Does not pass: naming a printed outcome instead of a spot, or arguing that either defect should have brought the run to a stop.", bloom: analyze, minChars: 60 }
socratic:
