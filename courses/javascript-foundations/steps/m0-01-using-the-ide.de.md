...
tasks:
  ...
  - id: where-things-are
    ...
    check: { type: question, prompt: { en: "Name the route you used, where the output appeared, and the file you edited. One sentence each.", de: "Nenne den benutzten Weg, wo die Ausgabe erschien, und die geänderte Datei. Je ein Satz." }, recallPrompt: { en: "Name the three routes this editor offers for running something, the region of the window their output appears in, and the folder your own edits belong in.", de: "Nenne die drei Wege, die dieser Editor zum Ausführen anbietet, den Bereich des Fensters, in dem ihre Ausgabe erscheint, und den Ordner, in den deine eigenen Änderungen gehören." }, rubric: "Drei Sätze, je einer pro Teil: welcher der drei Wege genommen wurde, welcher Bereich des Fensters das Ergebnis trug und in welchem der beiden Ordner die Änderung landete. Besteht nicht: eine Änderung im Ordner mit dem Prüfschema, weniger als drei beantwortete Teile, oder ein Bereich so ungenau benannt, dass er das ganze Fenster sein könnte.", bloom: understand, minChars: 60 }
socratic:
...
> expect: Am unteren Fensterrand öffnet sich ein Panel mit einem Prompt darin.
> recover: Antwortet die Palette *No matching results*, fehlt das führende `>`; ohne dieses Zeichen sucht die Palette nach Dateinamen statt nach Befehlen. Öffnet sich gar nichts, nimm **F1** statt Strg+Umschalt+P, das der Browser für sich behalten kann.
:::

Prüf, wo das Terminal steht:
