...
tasks:
  ...
  - id: default-trigger
    ...
    check: { type: question, prompt: { en: "For which single argument value does a default fire, and what does joinWords(null, \\\"a\\\") return?", de: "Bei welchem einzigen Argumentwert greift ein Standardwert, und was liefert joinWords(null, \\\"a\\\")?" }, rubric: "Nennt den einen Wert, auf den ein Parameterstandard anspringt, und führt den null-Aufruf bis zu einem Ergebnis durch, in dem keinerlei Zeichen zwischen etwas steht - hergeleitet aus der Zahl der Zwischenräume, die eine einelementige Liste bietet, nicht daraus, dass der Standard gegriffen hätte. Besteht nicht: mehr als ein auslösender Wert, ein Ergebnis, das den Standard voraussetzt, oder eines, in dem ein Trennzeichen erscheint, obwohl auf einer Seite nichts steht.", bloom: analyze, minChars: 40 }
socratic:
