...
tasks:
  ...
  - id: default-trigger
    ...
    check: { type: question, prompt: { en: "For which single argument value does a default fire, and what does joinWords(null, \\\"a\\\") return?", de: "Bei welchem einzigen Argumentwert greift ein Standardwert, und was liefert joinWords(null, \\\"a\\\")?" }, rubric: "Der auslösende Wert ist der eine, der für „nichts wurde übergeben“ steht, und ein weggelassenes Argument kommt genau als er an. Der null-Aufruf greift deshalb nicht auf den Standard zurück: null wird als Trennzeichen genommen, und da ein einzelnes Wort keine Lücke zu füllen hat, ist das Ergebnis dieses Wort allein. Besteht nicht: mehr als ein auslösender Wert, ein Ergebnis, das nur folgt, wenn der Standard gegriffen hätte, oder ein Ergebnis, in dem das Trennzeichen auftaucht, obwohl nichts zu trennen ist.", bloom: analyze, minChars: 40 }
socratic:
