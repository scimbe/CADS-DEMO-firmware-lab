...
tasks:
  ...
  - id: what-your-tests-found
    ...
    check: { type: question, prompt: { en: "Which cases did you add, and why does the given suite miss them? One sentence each.", de: "Welche Fälle hast du ergänzt, und warum fehlen sie der vorgegebenen Suite? Je ein Satz." }, rubric: "Two or more cases the given fixtures cannot produce - a boundary of the input, a shape the sample data never takes, or a failure path nothing in the suite triggers - each with the reason it lies outside that suite's reach. Then an honest outcome per case: a defect that surfaced, or an implementation that already held. Does not pass: a case the given suite already covers, or a claim that a test found something without saying what changed as a result.", bloom: evaluate, minChars: 80 }
socratic:
...
| `loadReport` | await a reader, then the three above | [m6-02](step:m6-02-async-await), [m6-03](step:m6-03-async-errors) |
