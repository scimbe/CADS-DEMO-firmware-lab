...
tasks:
  ...
  - id: two-errors
    ...
    check: { type: question, prompt: { en: "Both errors mention a name. Why is one a TypeError and the other a ReferenceError? Two sentences.", de: "Beide Fehler nennen einen Namen. Warum ist einer ein TypeError, der andere ein ReferenceError? Zwei Sätze." }, rubric: "Ordnet die beiden nach dem Ziel der Beanstandung: die eine beanstandet die Operation selbst, gerichtet auf einen Namen, dessen Wert endgültig feststeht; die andere beanstandet die Erwähnung, weil der Name noch nicht gelesen werden darf. Sagt, welche der beiden schon die Formulierung allein entscheidet. Besteht nicht: die zweite als einen Namen behandeln, den niemand deklariert hat, oder die beiden Reparaturen nennen, ohne zu sagen, worauf jede Beanstandung zielt.", bloom: analyze, minChars: 60 }
socratic:
