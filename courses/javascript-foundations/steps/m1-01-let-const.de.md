...
tasks:
  ...
  - id: two-errors
    ...
    check: { type: question, prompt: { en: "Both errors mention a name. Why is one a TypeError and the other a ReferenceError? Two sentences.", de: "Beide Fehler nennen einen Namen. Warum ist einer ein TypeError, der andere ein ReferenceError? Zwei Sätze." }, rubric: "Ein Satz sagt, dass die Bindung existiert und einen zweiten Wert verweigert, der Fehler also an der Operation liegt. Der andere sagt, dass die Bindung existiert, aber noch nicht gelesen werden darf, der Fehler also an der Referenz liegt. Besteht nicht: eine Antwort, die den zweiten Fall als nicht deklarierten Namen behandelt, oder eine, die die Reparaturen nennt, ohne zu sagen, worum es bei jeder Fehlerklasse geht.", bloom: analyze, minChars: 60 }
socratic:
