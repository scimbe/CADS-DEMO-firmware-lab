...
tasks:
  ...
  - id: which-iteration
    ...
    check: { type: question, prompt: { en: "Name what each of the four ways to walk a collection hands you. One line each.", de: "Nenne, was jeder der vier Wege durch eine Sammlung dir gibt. Je eine Zeile." }, rubric: "Vier Zeilen, jede nennt eine andere Art von Ding, die der Schleifenrumpf bekommt: einen Zähler, der vor der Verwendung erst aufgelöst werden muss; eine Bezeichnung, die als Text ankommt und auch eine sein kann, die niemand in die Folge gelegt hat; den Eintrag selbst; und den Eintrag zusammen mit seiner Stelle. Besteht nicht: zwei Zeilen, die dieselbe Art von Ding liefern, eine Zeile, die sagt, wozu eine Konstruktion nützlich ist, statt was sie liefert, oder eine Zeile, die diese Bezeichnungen als Zahlen liest.", bloom: understand, minChars: 60 }
socratic:
