...
tasks:
  ...
  - id: read-the-symptom
    ...
    check: { type: question, prompt: { en: "Why is reading one element too far easier to find than reading one too few? Two sentences.", de: "Warum ist ein Element zu weit zu lesen leichter zu finden als eines zu wenig? Zwei Sätze." }, recallPrompt: { en: "For an array of length 3, `a[3]` returns `undefined` with no error, but `a[3].toString()` throws `TypeError: Cannot read properties of undefined`. Reading one element too few instead just produces a shorter result with no error at all. Why is the one-too-far mistake easier to notice than the one-too-few mistake?", de: "Bei einem Array der Länge 3 liefert `a[3]` `undefined` ohne Fehler, aber `a[3].toString()` wirft `TypeError: Cannot read properties of undefined`. Ein Element zu wenig zu lesen liefert dagegen nur ein kürzeres Ergebnis, ganz ohne Fehler. Warum fällt der Zu-weit-Fehler leichter auf als der Zu-wenig-Fehler?" }, rubric: "Trennt einen Fehlschlag, den die Laufzeit ankündigt, von einem, den sie verschweigt: über das Ende hinaus liefert der Zugriff einen Wert ohne Eigenschaften, sodass der nächste Zugriff darauf das Programm sofort anhält, während ein Element zu wenig ein wohlgeformtes, nur kürzeres Ergebnis liefert, an dem niemand Anstoß nimmt. Schließt daraus, dass allein ein Fall genau auf der Grenze den zweiten aufdeckt. Besteht nicht: eine Antwort, in der schon der Zugriff über das Ende hinaus wirft, oder eine, die das Schweigen beim kürzeren Ergebnis nicht erklärt.", bloom: analyze, minChars: 60 }
socratic:
