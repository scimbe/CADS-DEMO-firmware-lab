...
tasks:
  ...
  - id: read-the-symptom
    ...
    check: { type: question, prompt: { en: "Why is reading one element too far easier to find than reading one too few? Two sentences.", de: "Warum ist ein Element zu weit zu lesen leichter zu finden als eines zu wenig? Zwei Sätze." }, rubric: "Erklärt, dass ein Lesen über das Ende hinaus einen Wert ohne Eigenschaften liefert, sodass die nächste Operation darauf das Programm anhält, während ein Element zu wenig einfach ein kürzeres Ergebnis liefert, das niemand ankündigt. Nennt die Folge: nur ein Test auf der Grenze fängt den zweiten Fall. Besteht nicht: eine Antwort, die sagt, der erste Fall werfe sofort, oder eine, die nicht sagt, warum der zweite still bleibt.", bloom: analyze, minChars: 60 }
socratic:
