...
tasks:
  ...
  - id: hoisting
    ...
    check: { type: question, prompt: { en: "One name was reachable from the top of the file and one was not. Why? Two sentences.", de: "Ein Name war vom Dateianfang erreichbar, einer nicht. Warum? Zwei Sätze." }, recallPrompt: { en: "A `function` declaration and an arrow function assigned to a `const`, both called from the line above their definition. Which call works, which does not, and why?", de: "Eine `function`-Deklaration und eine Pfeilfunktion, die an ein `const` gebunden ist, beide von der Zeile über ihrer Definition aufgerufen. Welcher Aufruf geht, welcher nicht, und warum?" }, rubric: "Trennt die beiden nach dem Augenblick, in dem ihr Name benutzbar wird, und knüpft diesen Augenblick an die Schreibweise, in der jede notiert ist. Wendet das auf die beobachtete Verweigerung an: sagt, welche Erwähnung ihrer eigenen Definition vorausgehen durfte und was die andere daran hindert. Besteht nicht: eine Antwort, die darauf beruht, das eine sei eine Funktion und das andere nicht, oder eine, die eine Zeile verschiebt, ohne die Eigenschaft zu benennen, die das Verschieben nötig machte.", bloom: analyze, minChars: 50 }
socratic:
