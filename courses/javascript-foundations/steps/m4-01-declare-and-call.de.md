...
tasks:
  ...
  - id: hoisting
    ...
    check: { type: question, prompt: { en: "One name was reachable from the top of the file and one was not. Why? Two sentences.", de: "Ein Name war vom Dateianfang erreichbar, einer nicht. Warum? Zwei Sätze." }, rubric: "Unterscheidet eine Form, die im ganzen Gültigkeitsbereich vollständig verfügbar ist, von einer Form, die erst lesbar wird, wenn ihre eigene Zeile gelaufen ist, und verbindet das mit der Reihenfolge, in der Modulanweisungen ausgeführt werden. Besteht nicht: eine Antwort, die sagt, das eine sei eine Funktion und das andere nicht, oder eine, die die Reparatur nennt, ohne den Unterschied zwischen den beiden Formen zu benennen.", bloom: analyze, minChars: 50 }
socratic:
