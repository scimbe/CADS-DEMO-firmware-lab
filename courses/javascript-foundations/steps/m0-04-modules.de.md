...
tasks:
  ...
  - id: named-vs-default
    ...
    check: { type: question, prompt: { en: "Why does a mistyped named import fail before any of your code runs? Two sentences.", de: "Warum scheitert ein vertippter benannter Import, bevor dein Code läuft? Zwei Sätze." }, recallPrompt: { en: "A test file writes `import meta, { square, cube } from \"../src/m0/math-utils.js\"`, but the module never exports `cube`. Node refuses to run a single test - no failing test, just a refusal to load. Why does that failure happen before any of your code runs?", de: "Eine Testdatei schreibt `import meta, { square, cube } from \"../src/m0/math-utils.js\"`, aber das Modul exportiert `cube` nicht. Node weigert sich, auch nur einen Test laufen zu lassen - kein fehlgeschlagener Test, nur eine Weigerung zu laden. Warum passiert dieser Fehlschlag, bevor überhaupt Code läuft?" }, rubric: "Verortet den Fehlschlag vor dem Beginn der Auswertung und knüpft das an das, was der Lauf gezeigt hat: nichts wurde als gescheiterter Test gemeldet, weil nichts angefangen hatte. Nennt den Ausgang, den der andere Zeitpunkt geliefert hätte - ein Name, dessen Fehlen erst auffällt, wenn ihn etwas benutzt. Besteht nicht: „weil es ein Tippfehler ist“; eine Antwort, die die Reparatur statt des Zeitpunkts nennt; oder eine, die einen gescheiterten Test genauso aussehen ließe.", bloom: understand, minChars: 50 }
socratic:
