...
tasks:
  ...
  - id: named-vs-default
    ...
    check: { type: question, prompt: { en: "Why does a mistyped named import fail before any of your code runs? Two sentences.", de: "Warum scheitert ein vertippter benannter Import, bevor dein Code läuft? Zwei Sätze." }, rubric: "Stellt fest, dass die Importe eines ES-Moduls beim Verknüpfen des Modulgraphen aufgelöst werden, bevor irgendein Modulrumpf ausgewertet wird, und dass ein importierter, nie exportierter Name deshalb ein SyntaxError zur Ladezeit ist. Stellt dem einen Wert gegenüber, der sich erst beim Aufruf als undefined erwiese. Besteht nicht: „weil es ein Tippfehler ist“, oder eine Antwort, die die Reparatur beschreibt, ohne den Zeitpunkt des Scheiterns zu nennen.", bloom: understand, minChars: 50 }
socratic:
