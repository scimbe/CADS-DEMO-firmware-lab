...
tasks:
  ...
  - id: mutate-or-copy
    ...
    check: { type: question, prompt: { en: "Sort the calls you used and replaced into two groups, then say when a mutating design is right.", de: "Ordne die benutzten und ersetzten Aufrufe in zwei Gruppen und sag, wann Verändern richtig ist." }, rubric: "Zwei Gruppen, die tatsächlich getrennt sind, plus ein vertretbarer Fall für das Verändern, etwa eine Sammlung, die die Funktion selbst erzeugt hat, oder eine so große, dass das Kopieren kostet. Ergänzt, was einem Aufrufer gesagt werden muss. Besteht nicht: eine Einteilung, die einen nicht verändernden Aufruf in die verändernde Gruppe steckt, oder ein Fall für das Verändern, bei dem es nur um weniger Tippen geht.", bloom: evaluate, minChars: 60 }
socratic:
