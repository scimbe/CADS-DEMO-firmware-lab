...
tasks:
  ...
  - id: three-operators
    ...
    check: { type: question, prompt: { en: "Give one case where each of the three comparisons is right, one sentence each, and one you would ban.", de: "Nenne für jeden der drei Vergleiche einen richtigen Fall, je ein Satz, und einen, den du verbieten würdest." }, recallPrompt: { en: "`==`, `===` and `Object.is`: give one case where each is the right choice, one sentence each, and name the one you would ban from a codebase.", de: "`==`, `===` und `Object.is`: nenne für jeden einen Fall, in dem er die richtige Wahl ist, je ein Satz, und den, den du in einer Codebasis verbieten würdest." }, rubric: "Drei Fälle plus ein Verbot mit Begründung. Jede vertretbare Aufteilung wird angenommen, sofern jeder Fall den genannten Operator wirklich verlangt und das Verbot aus Lesbarkeit oder aus dem Umwandlungsverhalten des Operators begründet wird. Besteht nicht: drei Fälle, die jeder der Operatoren erfüllen würde, ein Verbot ohne Begründung, oder der Operator für den Nicht-Zahl-Fall als Alltagsvorgabe benannt.", bloom: evaluate, minChars: 80 }
socratic:
