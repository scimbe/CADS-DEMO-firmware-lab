...
tasks:
  ...
  - id: pipeline-or-loop
    ...
    check: { type: question, prompt: { en: "Give one reason to prefer the method chain here, and one case where a loop wins.", de: "Nenne einen Grund für die Methodenkette hier und einen Fall, in dem eine Schleife gewinnt." }, rubric: "Der Grund für die Kette nennt etwas, das die Leserin gewinnt - eine benennbare Aufgabe je Stufe, kein Akkumulator, den man mitführen muss -, nicht Kürze. Der Schleifenfall nennt eine Fähigkeit, die der Kette fehlt: das frühe Verlassen beim ersten Treffer, oder ein einziger Durchgang, wo die Kette je Stufe einen über große Eingaben macht. Besteht nicht: eine Vorliebe ohne Begründung, oder ein Schleifenfall, den die Kette genauso gut bedient.", bloom: evaluate, minChars: 50 }
socratic:
...
misconceptions:
  ...
  - pattern: "max: 9\\b"
    question: { en: "The largest amount came out as the smallest of the three. Did a sort without a comparator decide that?", de: "Der größte Betrag kam als der kleinste der drei heraus. Hat eine Sortierung ohne Vergleichsfunktion das entschieden?" }
    hints: [ { en: "Array.prototype.sort with no argument compares the string forms of its elements, so 100 lands before 9.", de: "Array.prototype.sort ohne Argument vergleicht die Zeichenkettenform seiner Elemente, deshalb landet 100 vor 9." }, { en: "The same trap reorders the labels: a label order that is alphabetical rather than by size is this comparator missing.", de: "Dieselbe Falle vertauscht die Bezeichnungen: eine Reihenfolge, die alphabetisch statt nach Größe ist, ist genau diese fehlende Vergleichsfunktion." }, { en: "Numbers need a numeric comparator, or a fold that keeps the larger of two values - neither of which converts anything.", de: "Zahlen brauchen eine numerische Vergleichsfunktion oder eine Faltung, die den größeren zweier Werte behält - beides wandelt nichts um." } ]
  - pattern: "max: -Infinity"
    question: { en: "The empty report answered with something no amount could ever be. Which call has no neutral value to fall back on?", de: "Der leere Bericht antwortete mit etwas, das kein Betrag je sein kann. Welcher Aufruf hat keinen neutralen Wert, auf den er zurückfallen kann?" }
    hints: [ { en: "Math.max called with no arguments answers -Infinity, because that is the neutral element of a maximum.", de: "Math.max ohne Argumente aufgerufen antwortet -Infinity, weil das das neutrale Element eines Maximums ist." }, { en: "Spreading an empty array into a call passes no arguments at all, and the empty input is what the second assertion checks.", de: "Ein leeres Array in einen Aufruf zu spreizen übergibt gar keine Argumente, und die leere Eingabe ist genau das, was die zweite Zusicherung prüft." }, { en: "Either handle the empty case before the call, or fold with the starting value the exercise's contract names.", de: "Behandle entweder den leeren Fall vor dem Aufruf, oder falte mit dem Startwert, den der Auftrag der Aufgabe nennt." } ]
