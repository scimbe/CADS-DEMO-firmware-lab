...
tasks:
  ...
  - id: choose-combinator
    ...
    check: { type: question, prompt: { en: "Give one situation for each combinator and one that must run in order. One sentence each.", de: "Nenne je eine Situation für jeden Kombinator und eine, die nacheinander laufen muss. Je ein Satz." }, recallPrompt: { en: "`Promise.all` rejects immediately if any one job fails and gives back nothing from the others; `Promise.allSettled` always resolves and reports which jobs succeeded and which failed. Name one real situation that calls for each of the two, and one situation where the jobs must run one after another instead of concurrently at all.", de: "`Promise.all` schlägt sofort fehl, wenn eine Aufgabe scheitert, und liefert von den anderen nichts zurück; `Promise.allSettled` löst immer auf und meldet, welche Aufgaben gelungen und welche gescheitert sind. Nenne je eine reale Situation, die für die beiden spricht, und eine, in der die Aufgaben stattdessen nacheinander statt gleichzeitig laufen müssen." }, rubric: "Three situations that could not be swapped: one where a partial result is worthless, one where every outcome must be reported, and one where overlapping is impossible or forbidden. Does not pass: a sequential case that is only about preference, or two situations either combinator would serve equally.", bloom: evaluate, minChars: 60 }
socratic:
