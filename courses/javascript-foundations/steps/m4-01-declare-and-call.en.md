...
tasks:
  ...
  - id: hoisting
    ...
    check: { type: question, prompt: { en: "One name was reachable from the top of the file and one was not. Why? Two sentences.", de: "Ein Name war vom Dateianfang erreichbar, einer nicht. Warum? Zwei Sätze." }, recallPrompt: { en: "A `function` declaration and an arrow function assigned to a `const`, both called from the line above their definition. Which call works, which does not, and why?", de: "Eine `function`-Deklaration und eine Pfeilfunktion, die an ein `const` gebunden ist, beide von der Zeile über ihrer Definition aufgerufen. Welcher Aufruf geht, welcher nicht, und warum?" }, rubric: "Separates the pair by the instant each name turns usable, and pins that instant to the notation each was written in. Applies it to the observed refusal: says which mention could precede its own definition, and what stops the other from doing so. Does not pass: an answer resting on one being a function and the other not, or one that relocates a line without naming the property that made relocation necessary.", bloom: analyze, minChars: 50 }
socratic:
