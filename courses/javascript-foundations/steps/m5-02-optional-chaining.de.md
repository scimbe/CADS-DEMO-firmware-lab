...
tasks:
  ...
  - id: chaining-limits
    ...
    check: { type: question, prompt: { en: "Name one case the guarded path does not handle, and what you would write there instead.", de: "Nenne einen Fall, den der abgesicherte Pfad nicht abdeckt, und was du dort stattdessen schreiben würdest." }, rubric: "Der Fall muss einer sein, den die Absicherung wirklich nicht erreicht, und die Alternative eine andere Konstruktion. Angenommen werden: eine Absicherung, die nur den unmittelbar folgenden Zugriff schützt und nicht den danach; ein Zwischenwert, der weder fehlt noch ein Objekt ist, sodass der Zugriff an einem Typ scheitert statt an Abwesenheit; ein vertippter Schlüssel, der stillschweigend den Ersatzwert liefert und den Tippfehler verdeckt. Besteht nicht: ein Fall, den der abgesicherte Pfad sehr wohl abdeckt, oder eine Alternative, die wieder dieselbe Konstruktion ist.", bloom: evaluate, minChars: 50 }
socratic:
