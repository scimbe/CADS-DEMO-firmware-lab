...
tasks:
  ...
  - id: chaining-limits
    ...
    check: { type: question, prompt: { en: "Name one case the guarded path does not handle, and what you would write there instead.", de: "Nenne einen Fall, den der abgesicherte Pfad nicht abdeckt, und was du dort stattdessen schreiben würdest." }, rubric: "Der genannte Fall muss einer sein, den die Absicherung nicht erreicht, und das Gegenmittel muss eine andere Konstruktion sein. Angenommen werden: eine Absicherung, die nur den unmittelbar folgenden Schritt abdeckt; ein Zwischenwert, der da ist, aber von der falschen Art; ein falsch geschriebener Schlüssel, der danach genau wie ein nie gesetzter aussieht. Besteht nicht: ein Fall, den die Absicherung sehr wohl abdeckt, oder ein Gegenmittel, das wieder dieselbe Konstruktion ist.", bloom: evaluate, minChars: 50 }
socratic:
