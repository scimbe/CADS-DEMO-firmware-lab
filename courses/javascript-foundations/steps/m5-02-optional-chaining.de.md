...
tasks:
  ...
  - id: chaining-limits
    ...
    check: { type: question, prompt: { en: "Name one case the guarded path does not handle, and what you would write there instead.", de: "Nenne einen Fall, den der abgesicherte Pfad nicht abdeckt, und was du dort stattdessen schreiben würdest." }, rubric: "Nennt eine echte Grenze und eine konkrete Alternative. Angenommen werden: eine Absicherung, die nur den unmittelbar folgenden Zugriff schützt, ein Zwischenwert, der weder fehlt noch ein Objekt ist, oder ein vertippter Schlüssel, der stillschweigend zum Standardwert wird. Besteht nicht: ein Fall, den der abgesicherte Pfad sehr wohl abdeckt, oder eine Alternative, die wieder dieselbe Konstruktion ist.", bloom: evaluate, minChars: 50 }
socratic:
