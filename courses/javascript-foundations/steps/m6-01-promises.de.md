...
tasks:
  ...
  - id: what-is-pending
    ...
    check: { type: question, prompt: { en: "wait(20) returns at once. What does the caller hold, and why can it not be read straight away?", de: "wait(20) kehrt sofort zurück. Was hält der Aufrufer, und warum ist es nicht direkt lesbar?" }, rubric: "Nennt das Objekt und den Zustand, in dem es sich befindet, zählt die Zustände auf, in die es übergehen kann, und begründet den Umweg mit dem einen Ausführungsstrang statt mit Konvention. Besteht nicht: eine Antwort, die nur sagt, der Wert sei noch nicht fertig, ohne Begründung, oder eine, die einen Zustand nennt, in den das Objekt nicht zurückkehren kann.", bloom: understand, minChars: 60 }
socratic:
