...
tasks:
  ...
  - id: two-errors
    ...
    check: { type: question, prompt: { en: "Both errors mention a name. Why is one a TypeError and the other a ReferenceError? Two sentences.", de: "Beide Fehler nennen einen Namen. Warum ist einer ein TypeError, der andere ein ReferenceError? Zwei Sätze." }, rubric: "Sorts the pair by the target of the complaint: one objects to the operation itself, aimed at a name whose value is fixed for good; the other objects to the mention, because the name is not yet available for reading. States which of the two the wording alone already decides. Does not pass: treating the second as a name nobody declared, or offering the two repairs without naming what each complaint is aimed at.", bloom: analyze, minChars: 60 }
socratic:
