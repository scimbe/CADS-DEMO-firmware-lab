...
tasks:
  ...
  - id: spot-missing-await
    ...
    check: { type: question, prompt: { en: "A missing await raises no error. Name two symptoms and where you would look first.", de: "Ein fehlendes await wirft nicht. Nenne zwei Symptome und wo du zuerst nachsiehst." }, rubric: "Zwei verschiedene Symptome, etwa der Behälter ausgegeben, wo ein Wert hingehörte, eine daraus gelesene Eigenschaft, die undefined liefert, eine Wahrheitsprüfung, die immer denselben Zweig nimmt, oder Arbeit, die in falscher Reihenfolge fertig wird. Dazu eine erste Stelle zum Nachsehen, die der Aufruf ist und nicht die Prüfzeile. Besteht nicht: zwei Formulierungen desselben Symptoms, oder eine erste Stelle, die die scheiternde Testzeile ist.", bloom: analyze, minChars: 50 }
socratic:
