...
tasks:
  - id: guess-order
    ...
    check: { type: predict, prompt: { en: "Read examples/m6-await-order.js. Write down the exact order of its nine printed lines.", de: "Lies examples/m6-await-order.js. Schreib die genaue Reihenfolge seiner neun Ausgabezeilen auf." }, then: { type: command, command: "node examples/m6-await-order.js", expectExitCode: 0, expectStdout: "timeout" }, rubric: "Stellt die vorhergesagte Reihenfolge der ausgegebenen gegenüber und nennt mindestens zwei verschobene Zeilen, samt der Regel, der jede Verschiebung folgt. Besteht nicht: die Ausgabe als Liste wiedergeben, ohne die berichtigte Erwartung zu nennen.", bloom: evaluate }
  - id: store
