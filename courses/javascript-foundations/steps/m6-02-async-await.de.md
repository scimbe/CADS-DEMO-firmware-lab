...
tasks:
  - id: guess-order
    ...
    check: { type: predict, prompt: { en: "Read examples/m6-await-order.js. Write down the exact order of its nine printed lines.", de: "Lies examples/m6-await-order.js. Schreib die genaue Reihenfolge seiner neun Ausgabezeilen auf." }, then: { type: command, command: "node examples/m6-await-order.js", seedMustFail: false, expectExitCode: 0, expectStdout: "timeout" }, recallPrompt: { en: "A line after an `await`, a line inside `setTimeout(…, 0)`, and a line after both in the same function. In which order do the three run?", de: "Eine Zeile nach einem `await`, eine Zeile in `setTimeout(…, 0)`, und eine Zeile nach beiden in derselben Funktion. In welcher Reihenfolge laufen die drei?" }, rubric: "Stellt die vorhergesagte Reihenfolge der ausgegebenen gegenüber und nennt mindestens zwei verschobene Zeilen, samt der Regel, der jede Verschiebung folgt. Besteht nicht: die Ausgabe als Liste wiedergeben, ohne die berichtigte Erwartung zu nennen.", bloom: evaluate }
  - id: store
  ...
  - id: spot-missing-await
    ...
    check: { type: question, prompt: { en: "A missing await raises no error. Name two symptoms and where you would look first.", de: "Ein fehlendes await wirft nicht. Nenne zwei Symptome und wo du zuerst nachsiehst." }, rubric: "Die Symptome sind beobachtbar und unterscheiden sich voneinander: der Behälter ausgegeben, wo ein Wert hingehörte, eine daraus gelesene Eigenschaft, die undefined liefert, eine Wahrheitsprüfung, die immer denselben Zweig nimmt, Arbeit, die in falscher Reihenfolge fertig wird. Nachzusehen ist beim Aufruf, der den Wert erzeugt hat, nicht bei der Zeile, die den Unterschied gemeldet hat. Besteht nicht: zwei Formulierungen desselben Symptoms, oder eine erste Stelle, die die scheiternde Testzeile ist.", bloom: analyze, minChars: 50 }
socratic:
