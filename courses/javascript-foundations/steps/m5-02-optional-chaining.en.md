...
tasks:
  ...
  - id: chaining-limits
    ...
    check: { type: question, prompt: { en: "Name one case the guarded path does not handle, and what you would write there instead.", de: "Nenne einen Fall, den der abgesicherte Pfad nicht abdeckt, und was du dort stattdessen schreiben würdest." }, rubric: "The case has to be one the guard genuinely cannot reach and the alternative a different construct. Passing cases: a guard that protects only the access immediately after it and not the next one; an intermediate value that is neither absent nor an object, so the access fails on a type rather than on absence; a mistyped key that quietly yields the fallback and hides the typo. Does not pass: a case the guarded path does handle, or an alternative that is the same construct once more.", bloom: evaluate, minChars: 50 }
socratic:
