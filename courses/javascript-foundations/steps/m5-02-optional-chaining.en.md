...
tasks:
  ...
  - id: chaining-limits
    ...
    check: { type: question, prompt: { en: "Name one case the guarded path does not handle, and what you would write there instead.", de: "Nenne einen Fall, den der abgesicherte Pfad nicht abdeckt, und was du dort stattdessen schreiben würdest." }, rubric: "The case named has to be one the guard cannot reach, and the remedy has to be a different construction. Accepted: a guard covering only the step right after it; a middle value that is present but of the wrong kind; a key spelt wrong, which then looks exactly like a key nobody set. Does not pass: a case the guard does cover, or a remedy that is the same construction once more.", bloom: evaluate, minChars: 50 }
socratic:
