...
tasks:
  - id: guess-console
    ...
    check: { type: predict, prompt: { en: "examples/m0-console.js prints six lines. Write down, line by line, what you expect - including the exact shape of the last one, which prints an object.", de: "examples/m0-console.js gibt sechs Zeilen aus. Schreib Zeile für Zeile auf, was du erwartest - einschließlich der genauen Form der letzten Zeile, die ein Objekt ausgibt." }, then: { type: command, command: "node examples/m0-console.js", expectExitCode: 0, expectStdout: "Hello, JavaScript" }, rubric: "Sets the prediction against the six printed lines and names at least one line that differed, with the rule that was wrong. Does not pass: 'I was right' with no line named, or a list of the output without saying which expectation it corrected.", bloom: evaluate }
  - id: two-arguments
    title: Warum in der ersten Zeile ein Leerzeichen steht
    check: { type: question, prompt: { en: "Where does the space in the first printed line come from? One sentence, plus one line of code without it.", de: "Woher kommt das Leerzeichen in der ersten ausgegebenen Zeile? Ein Satz, plus eine Codezeile ohne es." }, rubric: "States that console.log joins its arguments with a single space, and gives one working line that avoids it, by concatenation or a template literal. Does not pass: an answer that says the string contains the space, or a replacement line that still passes two arguments.", bloom: understand, minChars: 40 }
socratic:
