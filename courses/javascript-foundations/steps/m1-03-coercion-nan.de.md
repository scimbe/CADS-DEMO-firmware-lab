...
tasks:
  ...
  - id: why-silent
    ...
    check: { type: question, prompt: { en: "Neither bug threw. For each, name the single step where a wrong type slipped through.", de: "Keiner der Fehler warf. Nenne für jeden die eine Stelle, an der ein falscher Typ durchkam." }, rubric: "Verortet zwei Stellen, eine je Defekt, und jede ist ein Operator, kein ausgegebenes Ergebnis. Der erste ist ein Operator, dessen Bedeutung die übergebenen Arten bestimmen, er wechselt also lautlos die Aufgabe; der zweite ist ein Vergleich, dessen Urteil feststand, bevor irgendwelche Daten eintrafen, nichts konnte ihn also je erfüllen. Besteht nicht: ein ausgegebenes Ergebnis statt einer Stelle nennen, oder behaupten, einer der beiden Defekte hätte den Lauf anhalten müssen.", bloom: analyze, minChars: 60 }
socratic:
