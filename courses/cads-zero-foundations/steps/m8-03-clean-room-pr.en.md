...
That one proposal violates all four of the project's standing rules at once. The three tasks of this step take it apart: licence, pin choice, reviewability.
...
**3. Safety.** `docs/SAFETY.md` is binding, including for code you cannot run. PA13/PA14 and PH0/PH1 are never touched; PF0..7 and PG0..5 are pulled-up inputs and are **never** configured as outputs. `docs/SAFETY.md` section 3 gives the electrical reason, and whether it also binds code that never reaches a board is the second task of this step.

**4. The workflow.** The maintainer holds the hardware exclusively: contributors do not flash, reset or attach a debugger. You take `swarm-ready` issues (self-contained, hardware-free); `hardware-gate` items are the maintainer's. What else a reviewable PR has to carry — the accompaniments to the change itself — is listed in one paragraph of `docs/how-to/agent-workflow.md`. Walk that list against the submission above; the third task asks what is missing.
...
Three separate verdicts on the proposal above. First the licence: what is the smallest change that makes the code acceptable? Then the pin choice: does the "it is never flashed" argument hold? Finally the form: what is missing before a reviewer can even start? The final step asks you to make a change that passes this review yourself.
