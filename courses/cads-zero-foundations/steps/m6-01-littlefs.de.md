...
tasks:
  ...
  - id: erase-vs-watchdog
    ...
    check: { type: predict, prompt: { en: "Formatting the volume erases seven 128 KB sectors one after another, and each erase blocks the calling thread. Predict whether the IWDG resets the board while that runs.", de: "Ein Format löscht sieben 128-KB-Sektoren nacheinander, und jedes Löschen blockiert den aufrufenden Thread. Sage voraus, ob der IWDG das Board dabei zurücksetzt." }, then: { type: command, cwd: ".", command: "grep -n -B16 -E 'CADS_FLASH_ERASE_TIMEOUT_MS|CADS_IWDG_RELOAD_VALUE' modules/storage/src/cads_flash_stm32f4.c targets/itsboard/hal/hal_watchdog.c", expectExitCode: 0, expectStdout: "CADS_IWDG_RELOAD_VALUE 1000u" }, rubric: "Der Vergleich stellt zwei Zahlen nebeneinander: das Löschzeitlimit des Treibers (CADS_FLASH_ERASE_TIMEOUT_MS 4000 ms, mit dem Kommentar, dass RM0090 für den 128-KB-Sektor eine Worst-Case-Zeit unter 2 s nennt) und die IWDG-Periode (Vorteiler /64, RLR 1000, bei nominell 32 kHz also 2,0 s - der Quelltextkommentar nennt dort 2,048 s, was aus 32 kHz nicht folgt; siehe M4-04). Bestanden, wenn die Antwort nach dem Vergleich erklärt, warum sich das nicht widerspricht, und zwar mit beiden Hälften. Erstens: die Warteschleife in cads_flash_erase_sector() ist ein blankes while(FLASH->SR & FLASH_SR_BSY) - sie belegt die CPU vollständig, sperrt aber keine Interrupts und betritt keine kritische Sektion, der SysTick-Interrupt schlägt also weiter zu und der 1-kHz-Tick-Hook füttert den IWDG durchgehend. Zweitens: der Baustein ist Dual-Bank, das Volume liegt in Bank 2, und das Löschen von Bank 2 hält den Befehlsstrom aus Bank 1 nicht an - genau darauf beruht, dass der Handler überhaupt geholt und ausgeführt werden kann. Fehlt eine der beiden Hälften, besteht die Antwort nicht; die Formulierung, das Löschen blockiere nur den aufrufenden Thread und nicht die CPU, ist zu ungenau. Eine Vorhersage ja mit dieser Auflösung besteht ebenso wie eine Vorhersage nein.", bloom: understand }
  - id: why-safe
    title: Erkläre, warum ein Reflash das Volume nicht zerstören kann
    check: { type: question, prompt: { en: "Why can writing a new firmware image not erase the littlefs volume?", de: "Warum kann das Schreiben eines neuen Firmware-Images das littlefs-Volume nicht löschen?" }, recallPrompt: { en: "The board's file system sits in bank 2 of the internal flash from 0x08120000, the firmware in bank 1 from 0x08000000. Why can writing a new firmware image not erase the volume? One mechanism is enough.", de: "Das Dateisystem des Boards liegt in Bank 2 des internen Flash ab 0x08120000, die Firmware in Bank 1 ab 0x08000000. Warum kann das Schreiben eines neuen Firmware-Images das Volume nicht löschen? Ein Mechanismus genügt." }, rubric: "Ein Grund genügt, muss aber der Mechanismus sein: st-flash write löscht sektorweise nur den geschriebenen Bereich ab 0x08000000 und macht nie ein Mass-Erase; oder das Image wird vor dem Schreiben gegen 1 MB geprüft und kann nicht über Bank 1 hinauslaufen; oder der Chip ist Dual-Bank, Bank 2 ist also eine getrennte Löschdomäne. Verortet das Volume dabei in Bank 2 ab 0x08120000. Eine Antwort, die nur sagt, das Werkzeug fasse es nicht an, besteht nicht.", bloom: understand }
socratic:
...
Die dritte Tatsache hängt an keinem Werkzeug: **der Chip ist Dual-Bank.** Bank 2, in der das Volume liegt, ist eine eigene Löschdomäne — was dort gelöscht wird, erreicht Bank 1 nicht.
...
Daneben legst du die Zahl aus M4-04: der IWDG dieses Boards läuft mit Vorteiler /64 und Reload 1000 und damit bei nominell 32 kHz nach **2,0 s** ab (`targets/itsboard/hal/hal_watchdog.c`; der Kommentar dort nennt 2,048 s — welche der beiden Zahlen aus den angegebenen Werten folgt, hast du in M4-04 entschieden). Zwei Größenordnungen, die verdächtig dicht beieinanderliegen. Ob daraus ein Problem wird, ist die zweite Aufgabe dieses Steps. Sag deine Vorhersage hin, bevor du den Quelltext aufmachst — der Vergleich danach ist die Stelle, an der diese Aufgabe lehrt.
