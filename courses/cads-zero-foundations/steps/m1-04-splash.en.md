...
1. **Source.** The first check does not look for the token just anywhere in the file, but inside the argument list of the drawing call, between the font argument and the colour argument. A comment carrying the same word does not pass it; only text that is actually drawn.
