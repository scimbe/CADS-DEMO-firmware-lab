...
Why every line in that file is commented: the same RAM carries the 48 KB floor `scripts/check_ram_budget.py` guards (M4-02). Growing any lwIP buffer is a decision against something else. The second task of this step has you estimate the total first and look it up afterwards.

## Your task

Return the board to the console prompt if needed, run the M5 net gate `h 10`, and read what it reports: link state, speed, and the packet and byte counters. Then predict the stack's RAM price and compare against the two constants. Finally answer the question on how a caller learns about link-up.
