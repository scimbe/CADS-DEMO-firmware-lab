...
tasks:
  ...
  - id: bits-to-ports
    ...
    check: { type: question, prompt: { en: "Which adapter outputs does the mask 0x0301 drive high?", de: "Welche Adapter-Ausgänge treibt die Maske 0x0301 auf High?" }, recallPrompt: { en: "Bits 0 to 7 of an output mask go to GPIOD, bits 8 to 15 to GPIOE. Which adapter outputs does the mask 0x0301 drive high?", de: "Die Bits 0 bis 7 einer Ausgangsmaske gehen an GPIOD, die Bits 8 bis 15 an GPIOE. Welche Adapter-Ausgänge treibt die Maske 0x0301 auf High?" }, rubric: "0x0301 ist binär 0000 0011 0000 0001, also sind Bit 0, Bit 8 und Bit 9 gesetzt (1 + 256 + 512 = 769). Bits 0..7 gehen an GPIOD, Bits 8..15 an GPIOE, also gehen OUT0, OUT8 und OUT9 auf High und ihre LEDs leuchten. Die Antwort muss die Umrechnung zeigen, nicht nur drei Namen nennen. Wer die Ziffer 3 fälschlich als Bit 0 und 1 liest und auf OUT0, OUT1, OUT9 kommt, hat den Stellenwert der Hexziffer übersehen und besteht nicht.", bloom: apply }
  - id: bsrr-vs-odr
...
> recover: Kommt vom Board gar nichts zurück, steht es noch im Touchscreen-App-Baum und überhört einzelne Buchstaben — führ dann in einem gewöhnlichen Terminal (**☰ → `Terminal` → `New Terminal`**) einmal `python3 scripts/board_key.py quit` (das Skript meldet `| sent: quit`) aus, nicht in der Board-Konsole. Zeigt die Konsole einen gelben Hinweis, ist der serielle Port im Browser nicht freigegeben: `CaDS Board: Verbinden` erneut aufrufen.
:::
