...
tasks:
  ...
  - id: where-was-it
    ...
    check: { type: question, prompt: { en: "In which of the four areas did the task's output appear, and what does its last line say?", de: "In welchem der vier Bereiche erschien die Ausgabe des Tasks, und was sagt ihre letzte Zeile?" }, recallPrompt: { en: "You start a task in the window that computes a RAM margin. In which of the four areas does its output appear, and how does the last line begin?", de: "Du startest im Fenster einen Task, der eine RAM-Marge ausrechnet. In welchem der vier Bereiche erscheint seine Ausgabe, und womit beginnt die letzte Zeile?" }, rubric: "Die Ausgabe erschien unten im Terminal-Bereich, in einem eigenen Terminal, das den Namen des Tasks trägt — nicht im Tutor-Panel und nicht im Editor. Die letzte Zeile beginnt mit PASS und nennt die Marge in Byte. Ein Satz zum Ort, ein Satz zur Zeile.", bloom: apply }
socratic:
