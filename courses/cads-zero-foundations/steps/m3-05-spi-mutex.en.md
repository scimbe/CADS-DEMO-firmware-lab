...
tasks:
  - id: per-byte-count
    ...
    check: { type: question, prompt: { en: "The older driver arbitrates per byte. Where do its 307,200 MAC cycles per full frame come from?", de: "Der ältere Treiber arbitriert pro Byte. Woraus ergeben sich seine 307 200 MAC-Zyklen je Vollbild?" }, recallPrompt: { en: "A 480x320 panel is written in RGB565, and an older driver arbitrated the shared bus per byte. Where do its 307,200 MAC cycles per full frame come from?", de: "Ein 480×320-Panel wird in RGB565 beschrieben, und ein älterer Treiber arbitrierte den geteilten Bus pro Byte. Woher kommen seine 307 200 MAC-Zyklen je Vollbild?" }, rubric: "From the panel size times the bus's colour depth: 480 × 320 = 153,600 pixels, and the panel is written in RGB565, so two bytes per pixel - 307,200 bytes per full frame. Arbitrating per byte means exactly one MAC stop-and-start cycle per byte, hence 307,200 cycles. Answering 153,600 forgets the two bytes per pixel; answering 76,800 used the framebuffer's 4 bpp instead of the bus format.", bloom: analyze }
  - id: basepri-dma
    title: Explain why only the DMA produced a hang
    check: { type: question, prompt: { en: "BASEPRI read 0x50. Both interrupt priorities this firmware sets - the display DMA at 6, the two UARTs at 8 - are masked by that value. So why did only the DMA produce a hang?", de: "BASEPRI stand auf 0x50. Beide Interrupt-Prioritäten, die diese Firmware setzt - die Display-DMA auf 6, die beiden UARTs auf 8 -, sind damit maskiert. Warum erzeugte trotzdem nur die DMA einen Hänger?" }, recallPrompt: { en: "BASEPRI read 0x50. The display DMA sits at priority 6 and the two UARTs at 8, so with four priority bits all three are masked. Why did only the DMA produce a hang all the same?", de: "BASEPRI stand auf 0x50. Die Display-DMA liegt auf Priorität 6, die beiden UARTs auf 8 — bei vier Prioritätsbits sind damit alle drei gesperrt. Warum erzeugte trotzdem nur die DMA einen Hänger?" }, rubric: "Two steps, both required. First the arithmetic: with four priority bits, priority 6 is stored as 0x60 and priority 8 as 0x80, both numerically greater than 0x50, so all three of this firmware's real ISR handlers fall under the block - DMA2_Stream3 as much as USART3 and USART6. They were all masked; priority alone does not explain the difference. Second the wait chain: cads_hal_spi_wait() in targets/itsboard/hal/hal_spi.c spins on the flag cads_spi_dma_active, and DMA2_Stream3_IRQHandler is the only thing that clears it. Block precisely that handler and the loop waits for an event that can no longer occur - that is the hang. Nothing spins on the masked UART handlers, so there the block cost only characters. Comparing 6 directly against 0x50 skips the shift. Repeating that the DMA was masked, without naming the wait chain and the flag, does not answer the why that was asked.", bloom: analyze }
  - id: find-gate
...
The comment at the top of `FreeRTOSConfig.h` draws the consequence for drivers: any ISR that calls a `FromISR` API must carry a numerically **greater** priority number than 5. This firmware has **three** real ISR handlers, and all three comply:

| Handler | File | Priority |
|---|---|---|
| `DMA2_Stream3_IRQHandler` (display DMA) | `targets/itsboard/hal/hal_spi.c` | `NVIC_SetPriority(DMA2_Stream3_IRQn, 6u)` |
| `USART3_IRQHandler` (console UART) | `targets/itsboard/hal/hal_console.c` | 8 |
| `USART6_IRQHandler` (UART to the Wi-Fi co-processor) | `targets/itsboard/hal/hal_uart_wifi.c` | 8 |

Count them yourself rather than taking that on trust: `grep -rn 'void [A-Za-z0-9_]*_IRQHandler' targets modules apps`. The long list from `targets/itsboard/startup/vectors_stm32f429.c` does not count — those are weak aliases all pointing at `Default_Handler`, so precisely *not* implemented handlers.

That gives you the numbers the second task needs — and one ingredient more, because the numbers alone are not enough: all three handlers sit above the `0x50` threshold and were equally blocked during that hang. So look at what `cads_hal_spi_wait()` in `targets/itsboard/hal/hal_spi.c` spins on, and which handler clears that flag.
