...
tasks:
  - id: build-firmware
    title: Die Firmware baut für das Board und erzeugt eine echte ELF
  ...
  - id: two-build-dirs
    title: Begründe die zwei Build-Verzeichnisse
    check: { type: question, prompt: { en: "Why is one build/ directory not enough for both builds? Name the property of the artefacts that forbids it.", de: "Warum genügt ein einziges Verzeichnis build/ nicht für beide Builds? Nenne die Eigenschaft der Artefakte, die das verbietet." }, recallPrompt: { en: "The board build and the host build come from the same source tree. Why is a single build/ directory not enough for both? Name the property of the artefacts that forbids it.", de: "Board-Build und Host-Build entstehen aus demselben Quellbaum. Warum genügt ein einziges Verzeichnis build/ für beide nicht? Nenne die Eigenschaft der Artefakte, die das verbietet." }, rubric: "Die beiden Builds übersetzen für verschiedene Prozessorarchitekturen: der Board-Build mit arm-none-eabi-gcc für den Cortex-M4, der Host-Build mit dem Compiler des eigenen Systems. Die Objektdateien tragen dieselben Namen, enthalten aber unvereinbare Befehlssätze; in einem gemeinsamen Verzeichnis würden sie einander überschreiben, und der Linker bekäme Objekte, die nicht zueinander passen. Dazu hält ein Build-Verzeichnis genau einen CMake-Cache mit genau einer Toolchain, sodass sich zwei Presets darin schon deshalb ausschließen. Wer nur mit Übersichtlichkeit oder Ordnung antwortet, hat die Ursache nicht genannt und besteht nicht.", bloom: understand }
socratic:
