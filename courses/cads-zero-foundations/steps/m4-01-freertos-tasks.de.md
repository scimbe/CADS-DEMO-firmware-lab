...
tasks:
  ...
  - id: console-used
    ...
    check: { type: question, prompt: { en: "Your k run printed console_free. How many bytes of the console stack have actually been used?", de: "Dein k-Lauf hat console_free gedruckt. Wie viele Byte des Konsolen-Stacks sind damit tatsächlich benutzt?" }, recallPrompt: { en: "The console command k reports console_free = 2,800 bytes, and CADS_CONSOLE_STACK is declared as 1024 words. How many bytes of the console stack have actually been used?", de: "Der Konsolenbefehl k meldet console_free = 2 800 Byte, und CADS_CONSOLE_STACK ist mit 1024 Worten deklariert. Wie viele Byte des Konsolen-Stacks sind tatsächlich benutzt?" }, rubric: "Die Antwort nennt die eigene gemessene Zahl und die Rechnung dazu, nicht ein Beispiel aus dem Text. CADS_CONSOLE_STACK ist 1024 Wörter, ein Wort ist vier Byte, der Stack ist also 4096 Byte groß; benutzt sind 4096 minus dem gedruckten console_free. Wer console_free selbst als Verbrauch ausgibt, hat die Bedeutung des Namens übersehen: cads_thread_stack_free() liefert uxTaskGetStackHighWaterMark, also den kleinsten je gemessenen freien Rest, mal vier Byte. Eine Antwort ohne konkrete Zahl vom eigenen Board zählt nicht.", bloom: apply }
socratic:
...
Die Stacks, deren Lage dieser Step erklärt, sind genau die, die der Wächter aus `m3-04-stack-guard` bewacht.
