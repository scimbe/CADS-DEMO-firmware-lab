...
tasks:
  - id: navigated
    title: You have found the boundary in the real tree
...
The fastest way to open a file is `Ctrl`/`Cmd`+`P` and typing its name. You start the first task at the bottom of this panel with the **Check** button: it runs two searches in the firmware's root directory and shows you the output with line numbers. Those line numbers are the actual yield — they tell you where to jump in the editor.
...
That is the line the first task searches for, together with the counter-test: in `gui/`, `apps/` and `services/` not a single file includes the chip vendor's register header `stm32f4xx.h`. As long as both hold, the rule holds not only in the diagram but in the real tree.
...
1. Press **Check** on the first task and open, with `Ctrl`/`Cmd`+`P`, the file whose line appears in the output (`modules/toolbox/CMakeLists.txt`). Look at `PUBLIC` and `PRIVATE` side by side there, and in the explorer (`Ctrl`/`Cmd`+`Shift`+`E`) at the public `modules/toolbox/include/cads/toolbox/`, the private `modules/toolbox/src/` and the module README.
