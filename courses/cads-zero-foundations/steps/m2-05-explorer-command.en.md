...
tasks:
  - id: wired
    title: The dispatcher calls your handler, and it is in the built firmware
  ...
  - id: line-buffer
    title: Derive the behaviour at the end of the buffer
    check: { type: question, prompt: { en: "The console reads into a fixed char line[32]. What happens to a command whose line is longer, and how do you recognise it in the output?", de: "Die Konsole liest in ein festes char line[32]. Was geschieht mit einem Kommando, dessen Zeile länger ist, und woran erkennst du es an der Ausgabe?" }, recallPrompt: { en: "The bring-up console reads an input line into a fixed char line[32]. What happens to a command whose line is longer, and how do you recognise it in the output?", de: "Die Bring-up-Konsole liest eine Eingabezeile in ein festes char line[32]. Was geschieht mit einem Kommando, dessen Zeile länger ist, und woran erkennst du es an der Ausgabe?" }, rubric: "The buffer holds only 32 characters; anything beyond that no longer fits into the same line. The surplus bytes do not vanish — they are read as the start of a new input, and their first character lands in the dispatcher's switch again. Two things reveal it: the command acts on a truncated argument, so it echoes a mask parsed too short, and immediately afterwards the answer to a command nobody meant to type appears — usually the help, because an arbitrary character counts as unknown. An answer that merely says the rest is ignored does not pass: the second answer would then not appear.", bloom: analyze }
socratic:
