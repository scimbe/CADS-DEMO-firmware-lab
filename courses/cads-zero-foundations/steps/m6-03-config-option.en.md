...
tasks:
  ...
  - id: not-live
    ...
    check: { type: question, prompt: { en: "Of display.brightness, net.ip and wifi.ssid, which one does Reload config not put into effect?", de: "Welcher der Schlüssel display.brightness, net.ip und wifi.ssid wirkt nach Reload config nicht?" }, recallPrompt: { en: "On the panel, Settings triggers a reload of the configuration file. Which of the three keys display.brightness, net.ip and wifi.ssid does that not put into effect, and why not?", de: "Auf dem Panel löst Settings ein Reload der Konfigurationsdatei aus. Welcher der drei Schlüssel display.brightness, net.ip und wifi.ssid wird dadurch nicht wirksam, und warum nicht?" }, rubric: "wifi.ssid. display.* and net.* are applied by Reload config - the network address immediately if the link is up, otherwise at the next link-up. wifi.ssid and wifi.password only update what a later Settings Join WiFi would use; a reload on its own triggers no join. The answer needs that reasoning, not just the key name.", bloom: apply }
  - id: truncated
