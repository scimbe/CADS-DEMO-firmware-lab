...
tasks:
  - id: magenta-rect
    title: The test pattern calls fill_rect with the missing slot, and the firmware still builds
    check: { type: all, checks: [ { type: command, cwd: ".", command: "cc -E -P -Igui -Icore gui/cads_splash.c | paste -sd ' ' - | grep -qE 'cads_canvas_fill_rect[(][^;]*CadsColorMagenta'", expectExitCode: 0, bloom: apply }, { type: task, label: "CaDS: Build", expectExitCode: 0, bloom: apply } ] }
  - id: staging-banks
    title: Predict how many staging banks the flush uses
    check: { type: predict, prompt: { en: "cads_canvas_flush() converts the damaged region to RGB565 in bands and pushes each band over DMA. Predict how many staging banks it uses today, and what that implies about the blit. One number and one sentence on the coupling.", de: "cads_canvas_flush() wandelt die beschädigte Region in Bändern nach RGB565 und schiebt jedes Band per DMA hinaus. Sage voraus, wie viele Staging-Bänke es heute benutzt, und was daraus für das Blit folgt. Eine Zahl und ein Satz zur Kopplung." }, then: { type: command, cwd: ".", command: "grep -nE 'cads_stage|CADS_STAGE_ROWS|second bank' gui/canvas.c", expectExitCode: 0, expectStdout: "Dropped the second bank" }, rubric: "The comparison shows exactly one bank, cads_stage[CADS_STAGE_PIXELS], and the comment that explicitly rejects a second. Passes if the answer, after the comparison, names the coupling: a second bank only pays for itself if band N+1 can be expanded while band N is still going out over DMA, but cads_hal_display_blit() waits on the bus before returning, so the bank was free again anyway. Predicting two banks and then naming the reason for one passes.", bloom: apply }
  - id: palette-indirection
    title: Derive what a theme change costs
    check: { type: question, prompt: { en: "What would have to happen for your test pattern to appear in different colours without a single new drawing call? Argue it from the buffer's format.", de: "Was müsste geschehen, damit dein Testmuster ohne einen einzigen neuen Zeichenaufruf in anderen Farben erscheint? Begründe es aus dem Format des Puffers." }, recallPrompt: { en: "This firmware's framebuffer stores four bits per pixel. What would have to happen for an already drawn image to appear in different colours without a single new drawing call? Argue it from the buffer's format.", de: "Der Framebuffer dieser Firmware speichert vier Bit je Bildpunkt. Was müsste geschehen, damit ein bereits gezeichnetes Bild ohne einen einzigen neuen Zeichenaufruf in anderen Farben erscheint? Begründe es aus dem Format des Puffers." }, rubric: "Changing the palette table is enough. The framebuffer is indexed at four bits per pixel: each pixel stores not a colour but the number of a slot, and which colour that slot carries is only stated by the table with its sixteen entries. Re-assign those entries and the very same unchanged buffer shows different colours; no redrawing is needed, only another push to the panel. A theme change is therefore a table, not a rebuild. An answer that says every drawing call must be repeated with different colour arguments has missed the indirection and does not pass.", bloom: understand }
socratic:
  - { trigger: "question:palette-indirection:weak", question: { en: "What exactly does a pixel store in this buffer — a colour, or something else?", de: "Was genau speichert ein Pixel in diesem Puffer — eine Farbe oder etwas anderes?" }, hints: [ { en: "Four bits are not enough for a colour, but plenty for a small number.", de: "Vier Bit reichen nicht für eine Farbe, aber gut für eine kleine Zahl." }, { en: "Look at the enum of colour slots in gui/canvas.h and count how many there are.", de: "Sieh dir in gui/canvas.h das Enum der Farbslots an und zähl, wie viele es sind." }, { en: "Between the value in the buffer and the colour on the panel sits a table — and tables can be swapped.", de: "Zwischen dem Wert im Puffer und der Farbe auf dem Panel liegt eine Tabelle — und Tabellen kann man austauschen." } ] }
...
## You already know the 448 ms

The number this step keeps using — about 448 ms for a full screen — is one you computed yourself in **M0-04** from the self test's rate. Here you see what the canvas defends against with it.
