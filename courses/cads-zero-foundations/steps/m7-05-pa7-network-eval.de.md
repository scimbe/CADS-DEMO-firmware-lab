...
## Die Schranke zusichern, nicht nur zitieren

Dass der längste Ausfall ein Band ist und nicht ein Vollbild, hängt an einer einzigen Eigenschaft des Canvas: `cads_canvas_flush()` überträgt in Bändern von höchstens sechzehn Zeilen, und `cads_hal_display_blit()` claimt und gibt den Bus je Band frei. Diese Eigenschaft ist portabler Code, also auf dem Host prüfbar.

Die erste Aufgabe verlangt einen Unity-Fall in `tests/unit/test_canvas.c`, der einen Vollbild-Flush auslöst und danach über den Blit-Mitschnitt der aufzeichnenden HAL zusichert, dass **kein einzelner Blit** mehr als ein Band trägt — 480 × 16 Bildpunkte. Fällt die Bandzerlegung weg, kippt genau dieser Fall, und mit ihm die Zahl, mit der du gleich argumentierst.

Die Rate von 342 kpixel/s aus `docs/reference/measurements.md` gilt für den Teiler `/16`; die `/8`-Zeile der Tabelle hat auf dem Host keine Entsprechung, dieser Fall deckt sie also nicht ab.

## Die Damage-Entscheidung aus M5 zahlt hier

In **M5-04** hast du die einzelne Bounding-Box gegen eine Damage-Liste gerechnet und dort schon gelesen, dass eine Damage-Entscheidung zugleich eine Netzwerkentscheidung ist. Wie teuer genau, entscheidest du jetzt.
