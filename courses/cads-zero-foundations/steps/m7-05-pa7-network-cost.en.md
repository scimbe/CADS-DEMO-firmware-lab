...
title: What the PA7 time-slice costs the network
bloom: analyze
...
tasks:
  - id: band-bounds-blackout
    title: Assert that one band bounds the blackout
...
Measure, with the project's own tools, what the PA7 display/Ethernet time-slice actually costs the network. [The next step](step:m7-06-pa7-position) uses this cost to take a position on the fix the project chose not to apply.

## Assert the bound, do not merely quote it

That the longest blackout is one band rather than a whole frame hangs on a single property of the canvas: `cads_canvas_flush()` transfers in bands of at most sixteen rows, and `cads_hal_display_blit()` claims and releases the bus per band. That property is portable code, and therefore checkable on the host.

The first task asks for a Unity case in `tests/unit/test_canvas.c` that triggers a full-screen flush and then asserts, over the recording HAL's blit log, that **no single blit** carries more than one band — 480 × 16 pixels. Remove the band decomposition and exactly this case topples, and with it the number you are about to argue from.

The rate of 342 kpixel/s from `docs/reference/measurements.md` holds for the `/16` divider; the `/8` row of that table has no host equivalent, so this case does not cover it.

## The M5 damage decision pays here

In [M5-05](step:m5-05-dirty-rect-judge) you judged the single bounding box against a damage list, and read there that a damage decision is at the same time a network decision. How expensive exactly is what you measure here.
...
The first is a code task with a build-and-test button; the other two are free-text questions at the bottom of the step text, each with an answer field and a **Check** button beside it. **Run all checks** at the top of that tab grades the free-text ones at once. If one stays red, the **Show hint** button on that same task helps; its first tier asks about what most often goes wrong.

1. **Assert it.** Add the Unity case that pins the band bound to the recording HAL's blit log.
2. **The mechanism.** Why is the blackout bounded to one band rather than to a redraw? Take the computation from `gui/canvas.c` and say where the bus claim and release sit.
3. **The consequence.** Which traffic class cannot take the blackout, and what rule do you derive from that for protocols of your own?
