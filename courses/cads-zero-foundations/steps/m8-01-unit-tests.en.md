...
Run the host tests and confirm they pass. Then predict the number of test subjects and count them. Finally answer which lines a new subject needs for ctest to actually run it — in the last step of this module you write exactly that. The next step covers the part of the display that unit tests cannot see.
