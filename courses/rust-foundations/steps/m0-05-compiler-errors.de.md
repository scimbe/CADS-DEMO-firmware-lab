...
tasks:
  ...
  - id: method
    ...
    check: { type: "question", prompt: { en: "You repaired three E0308 errors with fewer than three edits. Two sentences: which part of a diagnostic told you which line to change, and why one edit removed more than one error.", de: "Du hast drei E0308-Fehler mit weniger als drei Änderungen behoben. Zwei Sätze: welcher Teil einer Diagnose dir sagte, welche Zeile zu ändern ist, und warum eine Änderung mehr als einen Fehler beseitigt hat." }, rubric: "Der erste Satz nennt einen bestimmten Teil der Diagnose und was er verraten hat - das Paar expected/found oder die zweite Markierung mit der Beschriftung `expected due to this`, die auf die Deklaration zeigt statt auf die Verwendung. Der zweite Satz sagt, dass einer der drei Fehler die Folge eines anderen war, dass das Beheben der Ursache also die Wirkung mit beseitigt hat. Besteht nicht: die help-Zeile als das nennen, was den Änderungsort verriet; alle vier Teile einer Diagnose aufzählen, ohne sich zu entscheiden; oder die Fehler als Dubletten ausgeben statt als Ursache und Wirkung.", recallPrompt: { en: "A rustc diagnostic marks two places, one with carets and one with dashes. Which of them tells you where to edit, and how can one edit remove more than one error?", de: "Eine rustc-Diagnose markiert zwei Stellen, eine mit Dachzeichen und eine mit Strichen. Welche sagt dir, wo du ändern musst, und wie kann eine Änderung mehr als einen Fehler beseitigen?" }, bloom: "analyze", minChars: 60 }
socratic:
...
Führe den Befehl der Aufgabe *Die reparierte Datei kompiliert und gibt den Satz aus* aus.
> expect: Der Befehl endet ohne Fehler, und seine Ausgabe enthält `Ada is 36 years old and 1 metre 62 tall.`.
