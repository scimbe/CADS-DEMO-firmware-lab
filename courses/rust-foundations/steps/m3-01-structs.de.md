---
id: m3-01-structs
title: "Structs: Werte, die zusammengehoeren"
bloom: apply
...
tasks:
  ...
  - id: update-syntax
    title: "Du kannst erklaeren, warum ..s verbraucht und ..*r nicht"
...
`(30, 50)` und `Rectangle { width: 30, height: 50 }` enthalten dieselben zwei Zahlen. Der Unterschied ist, dass die zweite Form nicht verdreht werden kann. Eine Funktion mit Tupelparameter muss dokumentieren, welches Element was ist, und dem Aufrufer vertrauen; eine Funktion mit `Rectangle` kann auf diese Weise gar nicht falsch aufgerufen werden. Kapitel 5.1 fuehrt dasselbe Argument, indem es eine `area`-Funktion durch beide Formen refaktoriert.
...
`derive` erzeugt Trait-Implementierungen mechanisch: `Debug` ermoeglicht die Ausgabe mit `{:?}`, `Clone` liefert `.clone()`, `PartialEq` liefert `==`. Die Tests brauchen alle drei. `pub` an der Struktur und an jedem Feld steuert die Sichtbarkeit getrennt - ein Feld ohne `pub` waere fuer den Test unsichtbar.

Beim Instanziieren wird jedes Feld genannt. Vorgaben gibt es nicht; laesst du eines weg, erhaeltst du `error[E0063]: missing field`.

## Kurzform der Feldinitialisierung

Traegt eine Variable bereits den Feldnamen, schreibe ihn einmal:
...
Kein Sonderfall fuer Konstruktoren - das gilt ueberall, wo die Namen uebereinstimmen.

## Struct-Update-Syntax und ihr Ownership-Haken

`..other` fuellt jedes nicht genannte Feld und muss zuletzt stehen:
...
Hier lohnt es sich, langsamer zu lesen. `..` **verschiebt** die Felder, die es uebernimmt. Hier sind beide verbleibenden Felder `u32` und damit `Copy`, es wird also nichts aus `*r` herausbewegt und das Rechteck des Aufrufers bleibt unberuehrt.
...
`name` ist ein `String` und wird tatsaechlich aus `s` herausbewegt, `s` wird also verbraucht - deshalb steht der Parameter per Wert da. Dasselbe an einem `&Student` waere `error[E0507]: cannot move out of a shared reference`, und die Loesung waere entweder Ownership zu nehmen oder dieses eine Feld zu klonen. Die Unterscheidung zwischen `Copy` und Nicht-`Copy` aus M1 leistet hier die ganze Arbeit.

## Deine Aufgabe

Implementiere die sechs Funktionen und erklaere dann den Unterschied zwischen `..s` und `..*r`. Als Naechstes: Enums, fuer Daten, die eine von mehreren Formen sind statt alle von mehreren Feldern.
