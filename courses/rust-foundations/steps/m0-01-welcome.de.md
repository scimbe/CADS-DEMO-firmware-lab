...
- **`Cargo.toml`** ist das Manifest: Paketname, Rust-Edition und die Liste der Abhaengigkeiten (hier: keine, mit Absicht - alles in diesem Kurs kommt aus der Standardbibliothek).
- **`src/`** enthaelt die Uebungen, eine Datei je Step, gruppiert in `m0/` … `m6/` und `project/`. **Du bearbeitest diese Dateien, und nur diese.**
- **`tests/`** enthaelt eine Datei je Step, benannt wie der Step: `tests/m0-02-first-test.rs`. Sie sind fertig und du aenderst sie nicht. Lies sie - sie sagen genau, was dein Code leisten muss.
- **`README.md`** ist die Karte des Ordners, samt der Verzeichnisse, die dir spaeter begegnen (`examples/`, `snippets/`, `repair/`, `samples/`).

## Der erste Schritt, konkret

Oeffne ein Terminal in diesem Ordner und fuehre aus:
...
`cargo build` kompiliert das Paket. Es gibt eine Zeile je Crate aus und endet mit `Finished`. Es gelingt, **obwohl noch nichts implementiert ist**: unfertige Uebungen sind `todo!()`, ein Makro, das sich als beliebiger Typ typpruefen laesst und abstuerzt, sobald es erreicht wird. Das Paket kompiliert also immer; fehlschlagen tun die Tests.

## Die Ausgabe von cargo lesen

Der erste Build laedt nichts herunter und dauert ein bis zwei Sekunden. Zu sehen ist:
...
`dev`-Profil heisst: keine Optimierung, volle Debug-Informationen - der richtige Kompromiss fuer einen Kurs. Alles, was cargo erzeugt, landet in `target/`, das von git ignoriert wird und jederzeit geloescht werden darf.

## Die Tests eines einzelnen Steps ausfuehren

`cargo test` fuehrt alles aus, auch die gut zwanzig Steps, die du noch nicht begonnen hast, und meldet entsprechend eine Wand aus Fehlschlaegen. Das ist erwartbar und hilft dir gerade nicht. Fuehre stattdessen **einen** Step aus:

```bash
cargo test --test m0-02-first-test
```

Der Name hinter `--test` ist der Dateiname in `tests/` ohne `.rs` und identisch mit der Step-ID. Der Tutor zeigt diesen Befehl fuer den Step an, an dem du gerade bist; raten musst du nie.

## Deine Aufgabe

Fuehre die beiden Befehle oben aus; die ersten beiden Checks bestaetigen das. Beantworte dann, wo die Tests des naechsten Steps liegen und wie du nur sie ausfuehrst. Der naechste Step bringt diese Tests zum Bestehen.
