...
tasks:
  - id: qmark
    title: "Die vier ?-Uebungen bestehen"
...
Bei `Ok` packt es aus und macht weiter. Bei `Err` kehrt es sofort aus der **umgebenden Funktion** zurueck und wandelt den Fehler unterwegs mit `From`. Das ist der ganze Operator, und die beiden Haelften der Beschreibung sind genau die beiden Arten, wie er nicht anwendbar sein kann.

## Wo er verfuegbar ist

`?` kehrt aus der Funktion zurueck, in der es steht, diese muss also `Result` liefern (oder `Option`, wo es `None` weiterreicht). In einer `main` oder einem Test mit Rueckgabetyp `()` ergibt es:

```text
error[E0277]: the `?` operator can only be used in a function that returns `Result` or `Option`
```

Die Loesung ist entweder, die Signatur zu aendern - es ist ueblich und in Ordnung, dass `main` ein `Result<(), Box<dyn Error>>` liefert - oder den Fehler hier mit `match` oder `expect` zu behandeln.

## Die Umwandlung ist die interessante Haelfte

`From::from(e)` bedeutet, dass `?` funktioniert, sobald sich der Fehler in den Fehlertyp der Funktion umwandeln laesst. In `double_parsed`, `parse_all` und `sum_lines` sind die Typen bereits identisch - jeder Fehlschlag ist ein `ParseIntError`, und genau das deklariert die Signatur - die Umwandlung ist also die triviale und `?` kostenlos.
...
Zwei Fehlschlaege verschiedener Art. Ein fehlendes `x` erzeugt **gar keinen Fehlerwert** - `split_once` liefert ein `Option` - es gibt also nichts fuer `?` zu wandeln, bis du mit `ok_or(None)` einen erzeugst. Und die Zahlen erzeugen zwar einen `ParseIntError`, aber der Fehlertyp der Funktion ist `Option<ParseIntError>`, und keine `From`-Implementierung verbindet die beiden - jedes Parsen braucht also `map_err(Some)` vor seinem `?`.

Das ist die ehrliche Grenze. Existiert die Umwandlung, ist `?` unsichtbar; existiert sie nicht, schreibst du sie einmal am Rand. Der naechste Step waehlt die dritte Moeglichkeit: einen Fehlertyp definieren und ihm die `From`-Implementierung geben, sodass `?` innerhalb deines Moduls ueberall wieder kostenlos wird.

## Anmerkung zum Stil

`?` macht den guten Pfad zum einzigen, den man liest. `sum_lines` ist eine `for`-Schleife, ein `trim`, ein Ueberspringen und ein `?`; die Fehlerbehandlung ist ein einzelnes Zeichen, und doch wird nichts ignoriert.

## Deine Aufgabe

Implementiere die vier Funktionen und erklaere dann die Regel, die entscheidet, wann `?` verfuegbar ist.
