...
tasks:
  - id: guess
    ...
    check: { type: "predict", prompt: { en: "examples/m5_unwrap_panic.rs indexes a slice, calls get with an out-of-range index, and parses two strings with expect. Which line ends the program, what does the panic message say, and what is the process exit code?", de: "examples/m5_unwrap_panic.rs indiziert einen Slice, ruft get mit einem Index außerhalb des Bereichs auf und parst zwei Zeichenketten mit expect. Welche Zeile beendet das Programm, was sagt die Panic-Meldung, und wie lautet der Exit-Code des Prozesses?" }, then: { type: "command", command: "cargo run --quiet --example m5_unwrap_panic", seedMustFail: false, expectExitCode: 101, expectStderr: "not a valid port: ParseIntError \\{ kind: InvalidDigit \\}", timeoutMs: 120000 }, rubric: "Sagt vorher, dass v.get(99) None ausgibt, ohne abzustürzen, dass das Parsen von 8080 gelingt und dass das Parsen von http die Zeile ist, die abstürzt, mit der expect-Meldung gefolgt von der Debug-Form des Fehlers. Der Exit-Code 101 ist die Einzelheit, die den meisten Vorhersagen fehlt, und gehört ausdrücklich genannt. Eine Panic bei v[1] oder bei get(99) vorherzusagen ist das falsche Modell, das benannt gehört.", recallPrompt: { en: "A program indexes a slice, calls get with an out-of-range index, and parses two strings with expect, one of which is not a number. Which call ends the program, and with what exit code?", de: "Ein Programm indiziert einen Slice, ruft get mit einem Index außerhalb des Bereichs auf und parst zwei Zeichenketten mit expect, von denen eine keine Zahl ist. Welcher Aufruf beendet das Programm, und mit welchem Exit-Code?" }, bloom: "evaluate" }
  - id: panics
  ...
  - id: which-contract
    title: "Du kannst zwischen den beiden Verträgen wählen und es begründen"
    check: { type: "question", prompt: { en: "Your caller reads the index out of a configuration file. Two sentences: which of element_at and element_at_opt you hand them, and what the other one would cost.", de: "Dein Aufrufer liest den Index aus einer Konfigurationsdatei. Zwei Sätze: welche von element_at und element_at_opt du ihm gibst, und was die andere kosten würde." }, rubric: "Bewertet wird die Antwort. Alle drei Kriterien müssen erfüllt sein: (1) sie wählt die Funktion, die ein Option liefert, und bindet die Wahl an die Herkunft der Zahl - ein Wert, der von außerhalb des Programms hereinkam, ist Eingabe, und schlechte Eingabe ist eine Bedingung, die der Aufrufer ohnehin behandeln muss; (2) sie benennt die Kosten konkret: der Prozess endet an einem Tippfehler eines Menschen, an einer Stelle, die der Aufrufer nicht erreicht und nicht in eine Meldung für den Verursacher verwandeln kann; (3) sie lässt die andere Funktion als richtig stehen für den Fall, dass die Zahl aus der eigenen Rechnung des Programms stammt. Besteht nicht: eine Wahl danach, welche kürzer oder angenehmer aufzurufen ist; eine Antwort, in der der Aufrufer die Panic abfängt oder behandelt; eine Antwort, die darauf hinausläuft, dass Panics schlecht seien.", recallPrompt: { en: "Your caller reads an index out of a configuration file. Which do you hand them, the function that panics or the one that returns an Option, and what would the other one cost?", de: "Dein Aufrufer liest einen Index aus einer Konfigurationsdatei. Welche gibst du ihm, die abstürzende Funktion oder die, die ein Option liefert, und was kostete die andere?" }, bloom: "evaluate", minChars: 60 }
socratic:
...
Sage das Beispiel vorher, implementiere die vier Funktionen so, dass jede genau dort und so abstürzt, wie die Tests es verlangen, und wähle dann in zwei Sätzen zwischen den beiden Verträgen. Der nächste Step liefert Fehler zurück, statt das Programm zu beenden.
...
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *Die vier Funktionen stürzen dort ab, wo sie sollen*.
> expect: Je Test eine Zeile `test … ok` oder `… FAILED`, darunter die Zusammenfassung `test result: ok. 8 passed; 0 failed`, sobald alle 8 bestehen. Der erste Lauf braucht ein paar Sekunden, weil die Crate einmal übersetzt wird; jeder weitere bleibt deutlich unter einer Sekunde.
