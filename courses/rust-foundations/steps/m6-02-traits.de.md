...
tasks:
  ...
  - id: default-method
    ...
    check: { type: "question", prompt: { en: "Tweet implements only author() yet Summary::summarize() works on it, and Article's own summarize() never calls the default. Explain how the compiler decides which body runs, and what the default method's call to self.author() means for a type that has not been written yet.", de: "Tweet implementiert nur author(), und dennoch funktioniert Summary::summarize() darauf, während Articles eigenes summarize() die Vorgabe nie aufruft. Erkläre, wie der Compiler entscheidet, welcher Rumpf läuft, und was der Aufruf self.author() in der Vorgabemethode für einen noch nicht geschriebenen Typ bedeutet." }, rubric: "Stellt fest, dass eine Methode im impl-Block die Vorgabe des Traits für diesen Typ ersetzt und dass ein Typ ohne eigenen Rumpf die Vorgabe bekommt - die Wahl fällt je Typ zur Übersetzungszeit, statisch, nicht durch ein Nachschlagen zur Laufzeit. Die zweite Hälfte hält fest, dass die Vorgabe die verlangte Methode über das Trait aufruft und daher für jeden künftigen Implementierer funktioniert, der author() liefert, was eine Vorgabemethode zu wiederverwendbarem Verhalten macht statt zu einer Kopie. Besteht nicht: antworten, die Vorgabe werde zuerst aufgerufen und dann überschrieben; oder ein Nachschlagen zur Laufzeit beschreiben - die Wahl fällt einmal je Typ zur Übersetzungszeit.", recallPrompt: { en: "A trait has one required method and one default method that calls it. How does the compiler decide which body runs for a type, and what does that call mean for a type not yet written?", de: "Ein Trait hat eine verlangte Methode und eine Vorgabemethode, die sie aufruft. Wie entscheidet der Compiler, welcher Rumpf für einen Typ läuft, und was bedeutet dieser Aufruf für einen noch nicht geschriebenen Typ?" }, bloom: "understand", minChars: 70 }
socratic:
  - { trigger: "task:default-method:failed", question: { en: "Look at the two impl blocks. Which one contains a summarize, and which one does not?", de: "Sieh dir die beiden impl-Blöcke an. Welcher enthält ein summarize, welcher nicht?" }, hints: [ { en: "A method written in an impl block is the one that exists for that type; the trait's version is what is there when the impl block is silent.", de: "Eine in einem impl-Block geschriebene Methode ist die, die für diesen Typ existiert; die Fassung des Traits ist das, was da ist, wenn der impl-Block schweigt." }, { en: "So ask when that decision is made: does anything have to be looked up while the program runs?", de: "Frage also, wann diese Entscheidung fällt: muss während der Programmausführung irgendetwas nachgeschlagen werden?" }, { en: "For the second half, note which method the default body calls, and whether that method is optional for an implementor.", de: "Beachte für die zweite Hälfte, welche Methode der Vorgaberumpf aufruft und ob diese Methode für einen Implementierer optional ist." } ] }
...
::: do command="cargo test --test m6-02-traits" cwd="."
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *Article überschreibt, Tweet nutzt die Vorgabe*.
