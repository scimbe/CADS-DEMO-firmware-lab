...
tasks:
  - id: custom
    title: "Der eigene Fehlertyp verhaelt sich richtig"
...
Definiere einen Fehlertyp fuer dein Modul, lass `?` automatisch dorthin wandeln und entscheide, was ueberhaupt ein Fehler sein soll.

## Warum nicht einfach `String`

`Result<T, String>` hat m5-02 benutzt, und fuer eine kleine Funktion ist das in Ordnung. Es hoert auf, in Ordnung zu sein, sobald ein Aufrufer je nach Fehlschlag etwas anderes *tun* will: ein `String` laesst sich ausgeben und sonst nichts. Ein Enum laesst sich matchen.
...
Zwei Varianten, jede mit dem Kontext, den ein Leser braucht. Ein Aufrufer kann nun sagen: "ein Syntaxfehler heisst, zeige die Zeile; eine falsche Zahl heisst, nimm die Vorgabe" - was keine Zeichenkette erlaubt.
...
`write!` schreibt in den Formatter und liefert das Ergebnis, das das Trait erwartet. `Display` zu implementieren liefert dir ausserdem `.to_string()` kostenlos.
...
Diese eine Zeile ermoeglicht es, dass ein `ConfigError` in ein `Box<dyn Error>` passt und durch Code nach oben reist, der dein Modul nicht kennt - der Test `config_error_is_a_std_error` prueft genau das.

## Das From-Impl holt `?` zurueck
...
Einmal geschrieben, laesst es jedes `?` in einer Funktion mit Rueckgabetyp `Result<_, ConfigError>` einen Parse-Fehlschlag automatisch wandeln. In `parse_config` lautet die Zeile schlicht:
...
Beachte, dass `e.to_string()` den Wortlaut des Parsers speichert, `invalid digit found in string`. Der Test sichert genau diesen Text zu, was den Kurs ehrlich haelt - es ist, was die Standardbibliothek auf dieser Maschine tatsaechlich erzeugt, und nichts fuer die Uebung Erfundenes.

## Nicht alles ist ein Fehler

`config_get` liefert `Ok(None)` fuer einen fehlenden Schluessel und `Err` nur, wenn die Datei selbst kaputt ist. Das sind wirklich verschiedene Lagen: ein fehlender optionaler Schluessel hat eine naheliegende Antwort, eine fehlerhafte Datei bedeutet, dass keinem Datum zu trauen ist. Beides zusammenzulegen zwaenge jeden Aufrufer, einen Fehler zu untersuchen, nur um herauszufinden, ob ueberhaupt etwas nicht stimmt.

## Deine Aufgabe

Implementiere `Display`, das `From`-Impl, `parse_config` und `config_get`, verteidige dann die `Ok(None)`-Entscheidung und nenne einen Aufrufer, fuer den sie falsch waere. Modul M6 ist das letzte vor dem Projekt.
