...
Run this step's tests. The same command sits behind the **Check** button on the *The library passes its tests* task.
> expect: One line per test, `test … ok` or `… FAILED`, then the summary `test result: ok. 9 passed; 0 failed` once all 9 pass. The first run takes a few seconds while the crate compiles once; every run after that stays well under a second.
...
Run the command of the *The command-line tool runs on the sample* task.
> expect: The command ends without an error and its output begins with `words: 24`.
...
Run the command of the *A missing file fails cleanly* task and read its message.
> expect: It **fails on purpose**: it ends with code 1 and its error output contains `wordstat: cannot read file:`. That is exactly what the check wants to see.
> recover: If it ends with code 0 instead, your implementation does not yet treat the failing case as a failure. If it reports something else, compare your text with the expected one character by character.
:::
