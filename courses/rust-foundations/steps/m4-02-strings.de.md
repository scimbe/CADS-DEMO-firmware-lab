...
estimatedMinutes: 25
scaffold: faded
recallFrom: [ "m4-01-vectors", "m2-04-slices" ]
links:
...
tasks:
  ...
  - id: no-index
    ...
    check: { type: "command", command: "mkdir -p target/check && rustc --edition 2024 --emit=metadata --out-dir target/check snippets/m4_02_string_index.rs", expectExitCode: 1, expectStderr: "the type `str` cannot be indexed by `\\{integer\\}`", timeoutMs: 120000 }
socratic:
