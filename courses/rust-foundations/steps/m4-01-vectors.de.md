...
tasks:
  ...
  - id: index-vs-get
    ...
    check: { type: "question", prompt: { en: "get_at must return None for an out-of-range index, so v[i] is wrong there. Name a place in the same file where v[i] would be the better choice, and state the rule you would give a colleague for picking between the two.", de: "get_at muss für einen Index außerhalb des Bereichs None liefern, v[i] ist dort also falsch. Nenne eine Stelle in derselben Datei, an der v[i] die bessere Wahl wäre, und formuliere die Regel, die du einem Kollegen für die Wahl zwischen beiden geben würdest." }, rubric: "Nennt eine Stelle, an der der Index nachweislich im Bereich liegt (ein aus der Länge des Vektors abgeleiteter Index oder eine Iteration), und formuliert eine Regel dieser Form: v[i], wenn ein Index außerhalb des Bereichs ein Fehler im Programm wäre - eine Panic ist dann die richtige, laute Antwort -, und v.get(i), wenn der Index von außen kommt und ein Wert außerhalb des Bereichs eine erwartete Bedingung ist, die der Aufrufer behandeln muss. Besteht nicht: eine Regel, die nur einen der beiden Fälle nennt; oder eine Regel aus Stil oder Gewohnheit statt aus der Herkunft des Index und der Frage, ob ein Wert außerhalb des Bereichs ein Fehler ist.", recallPrompt: { en: "A function must return None for an out-of-range index, so v[i] is wrong there. Name a case where v[i] would be the better choice, and state the rule for choosing between the two.", de: "Eine Funktion muss für einen Index außerhalb des Bereichs None liefern, v[i] ist dort also falsch. Nenne einen Fall, in dem v[i] die bessere Wahl wäre, und formuliere die Regel für die Wahl zwischen beiden." }, bloom: "evaluate", minChars: 60 }
socratic:
  - { trigger: "task:index-vs-get:failed", question: { en: "Where does each index in this file come from - your own code, or the function's caller?", de: "Woher kommt jeder Index in dieser Datei - aus deinem eigenen Code oder vom Aufrufer der Funktion?" }, hints: [ { en: "An index you computed from the collection's own length cannot be out of range unless you made an arithmetic mistake.", de: "Ein Index, den du aus der Länge der Sammlung selbst berechnet hast, kann nicht ausserhalb liegen, es sei denn du hast dich verrechnet." }, { en: "An index handed to you by a caller can be anything, and you cannot know whether they checked it.", de: "Ein Index, den ein Aufrufer übergibt, kann alles sein, und du kannst nicht wissen, ob er geprüft wurde." }, { en: "So the rule turns on one question: would being out of range mean the program has a bug, or that the input was simply unusual?", de: "Die Regel dreht sich also um eine Frage: würde ein Index ausserhalb bedeuten, dass das Programm einen Fehler hat, oder dass die Eingabe schlicht ungewöhnlich war?" } ] }
...
::: do command="cargo test --test m4-01-vectors" cwd="."
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *Die fünf Vektor-Funktionen bestehen*.
