...
Run this step's tests. The same command sits behind the **Check** button on the *All six struct functions pass* task.
> expect: One line per test, `test … ok` or `… FAILED`, then the summary `test result: ok. 6 passed; 0 failed` once all 6 pass. The first run takes a few seconds while the crate compiles once; every run after that stays well under a second.
