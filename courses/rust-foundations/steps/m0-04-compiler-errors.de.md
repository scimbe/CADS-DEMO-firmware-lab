...
Behandle eine rustc-Diagnose als strukturierten Bericht, auf den du reagieren kannst, statt als Textwand - und repariere eine Datei, die nicht kompiliert, ohne etwas zu aendern, das der Compiler nicht bemaengelt hat.

## Was eine Rust-Diagnose enthaelt
...
- **`error[E0308]`** ist ein stabiler Code. `rustc --explain E0308` gibt eine Seite dazu aus, der Online-Fehlerindex enthaelt denselben Text.
...
- **`help:`** schlaegt eine Aenderung vor. Sie ist oft richtig und loest gelegentlich das falsche Problem - sie sieht nur den lokalen Ausdruck, nicht deine Absicht.

## Die Datei

`repair/m0_04_type_mismatch.rs` gehoert nicht zum cargo-Paket; sie ist ein eigenstaendiges Programm, das du direkt uebersetzt. Im jetzigen Zustand erzeugt sie drei E0308-Fehler. Ihre Funktion `describe` ist korrekt und darf nicht angefasst werden; alle drei Fehler stehen in `main`.

Uebersetze und starte sie in einem Zug - genau das tut auch der Check:
...
Behebe **einen** Fehler und uebersetze neu. Fehler pflanzen sich fort: eine falsche Deklaration in Zeile 24 erzeugt eine zweite Beanstandung an der Aufrufstelle in Zeile 28, und die Korrektur der Deklaration beseitigt beide. Alle drei gleichzeitig zu jagen fuehrt meist zu einer Datei mit drei unabhaengigen Aenderungen, von denen zwei ueberfluessig waren.

Zwei Vorgaben halten das ehrlich. Behalte jede Variable - eine zu loeschen tauscht einen Typfehler gegen `E0425: cannot find value`. Und belasse den ausgegebenen Satz genau wie oben; der Check vergleicht ihn.

## Deine Aufgabe

Repariere die Datei, sodass sie kompiliert und den Satz ausgibt, und erklaere dann, welche Teile der Diagnose du tatsaechlich benutzt hast. Ab hier werden die Fehler interessanter: das naechste Modul behandelt Ownership, und seine Fehler drehen sich darum, *wann* ein Wert aufhoert, dir zu gehoeren.
