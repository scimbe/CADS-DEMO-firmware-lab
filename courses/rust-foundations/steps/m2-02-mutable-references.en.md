...
- `v.swap(0, last)`: the standard library's method takes two *indices* rather than two references. Count the mutable borrows that leaves.
...
Run this step's tests. The same command sits behind the **Check** button on the *change, append_twice and swap_ends pass* task.
