...
tasks:
  - id: bounds
    title: "Die vier beschraenkten Funktionen bestehen"
  ...
  - id: impl-vs-generic
    title: "Du kannst sagen, wann &impl Trait nicht genuegt"
...
Schreibe die drei Formen einer Trait-Schranke, kombiniere zwei Schranken und waehle die Form, die sagt, was du meinst.
...
Alle drei sagen "irgendein Typ, der `Summary` implementiert". `impl Trait` ist Zucker fuer die zweite; die `where`-Klausel ist die zweite unter die Signatur verschoben - dorthin gehoeren lange Schrankenlisten, bevor sie den Rueckgabetyp aus der Zeile draengen.

## Was `impl Trait` nicht kann

`&impl Summary` fuehrt **bei jedem Auftreten einen neuen anonymen Parameter** ein. Damit akzeptiert dies:
...
Dasselbe gilt fuer einen Slice: `&[T]` braucht den Namen, `summarize_all` und `longest_summary` koennen die Kurzform also gar nicht nutzen. Und ein anonymer Parameter hat keinen Namen, den man an der Aufrufstelle angeben koennte, `summarize_all::<Tweet>(&[])` - was der Test braucht, weil ein leerer Slice dem Compiler nichts zum Herleiten bietet - ist also nur mit der ausdruecklichen Form moeglich.

Faustregel: nimm `impl Trait` fuer ein einzelnes, einmal verwendetes Argument; benenne den Parameter, sobald er mehr als einmal auftritt.
...
`+` heisst "und". Der Rumpf gibt aus, was `Display` braucht, und vergleicht, was `PartialOrd` braucht. Verlange genau das, was der Rumpf nutzt: eine unnoetige Schranke weist Aufrufer ohne Gegenwert ab, und eine fehlende ist `error[E0277]`.
...
Jede Form hier wird monomorphisiert: der Compiler erzeugt je konkretem Typ eine Kopie, loest die Aufrufe zur Uebersetzungszeit auf und kann sie einbetten. Es gibt keine vtable und kein Nachschlagen zur Laufzeit. (`&dyn Summary` ist die andere Wahl, mit einer Kopie des Codes und einem Nachschlagen zur Laufzeit - ausserhalb des belegten Materials dieses Kurses, aber den Namen wert.)

## Deine Aufgabe

Implementiere die vier Funktionen - achte auf den dreiwertigen Vergleich in `describe_pair`, wo der Gleichheitsfall einen eigenen Zweig braucht - und nenne dann eine Signatur, die `impl Trait` nicht ausdruecken kann. Ein Step bleibt vor dem Projekt: Lifetimes.
