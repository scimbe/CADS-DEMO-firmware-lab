...
Run the command of the *The repaired file compiles and runs* task.
> expect: The command ends without an error and its output contains `Call me Ishmael`.
...
Run this step's tests. The same command sits behind the **Check** button on the *The lifetime exercises pass* task.
