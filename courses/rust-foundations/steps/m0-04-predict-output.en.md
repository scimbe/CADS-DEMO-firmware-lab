...
`examples/m0_shadowing.rs` is under `examples/`, a directory cargo compiles as standalone programs. One is run with `cargo run --example <name>`. Do not run this one yet: the check runs it for you, and only once your prediction is written down.
...
Run this step's tests. The same command sits behind the **Check** button on the *The temperature conversions pass* task.
