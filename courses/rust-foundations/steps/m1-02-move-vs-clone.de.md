...
tasks:
  ...
  - id: cost
    ...
    check: { type: "question", prompt: { en: "A reviewer calls the clone in duplicate wasteful. Two sentences: one caller for whom they are right, and why the test still forbids removing it.", de: "Ein Reviewer nennt den clone in duplicate verschwenderisch. Zwei Sätze: ein Aufrufer, für den er recht hat, und warum der Test das Entfernen dennoch verbietet." }, rubric: "Erster Satz: nennt einen Aufrufer, für den die Kopie Verschwendung ist - einen, der beide Ergebnisse nur liest, einen, der das zweite verwirft, oder einen mit einer sehr großen Zeichenkette, bei der das Kopieren der Bytes überwiegt. Zweiter Satz: der Test verändert das erste Ergebnis und sichert zu, dass das zweite unverändert bleibt, wofür zwei unabhängige Puffer nötig sind, und die liefert nur clone. Besteht nicht: die Kosten von clone aufsagen, ohne einen Aufrufer zu nennen; oder behaupten, der clone lasse sich im vorliegenden Code entfernen.", recallPrompt: { en: "A function clones its argument to return two independent Strings. Name a caller for whom that copy is waste, and a demand a test could make that no move-only version could satisfy.", de: "Eine Funktion klont ihr Argument, um zwei unabhängige Strings zu liefern. Nenne einen Aufrufer, für den diese Kopie Verschwendung ist, und eine Forderung eines Tests, die ohne Klon nicht erfüllbar wäre." }, bloom: "analyze", minChars: 50 }
socratic:
