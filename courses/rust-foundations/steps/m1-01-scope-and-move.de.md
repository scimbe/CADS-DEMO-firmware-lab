...
socratic:
  - { trigger: "task:guess:failed", question: { en: "Your prediction and what rustc reported do not agree. Did you expect it to compile at all, or is only the line you named wrong?", de: "Deine Vorhersage und das, was rustc meldet, gehen auseinander. Hattest du erwartet, dass es überhaupt kompiliert, oder stimmt nur die von dir genannte Zeile nicht?" }, hints: [ { en: "A String owns its buffer. Ask what `let s2 = s1;` does to that ownership when Rust neither copies the buffer nor counts owners: how many owners exist afterwards, and how many is it allowed to have?", de: "Ein String besitzt seinen Puffer. Frage, was `let s2 = s1;` mit diesem Besitz macht, wenn Rust den Puffer weder kopiert noch Eigentümer zählt: wie viele Eigentümer gibt es danach, und wie viele darf es geben?" }, { en: "Open `snippets/m1_01_move_error.rs` and read only the two `println!` lines. The compiler does not mark the line that moves; it marks the first line after it that still uses the old name.", de: "Öffne `snippets/m1_01_move_error.rs` und lies nur die beiden `println!`-Zeilen. Der Compiler markiert nicht die Zeile, die verschiebt, sondern die erste danach, die den alten Namen noch benutzt." }, { en: "Run the check again and find the three marks in its output: where the value was created, `value moved here`, and the borrow after the move. The line number in front of the third mark is the one to hold against yours, and the error code stands in square brackets in the very first line.", de: "Führe die Prüfung noch einmal aus und suche in ihrer Ausgabe die drei Marken: wo der Wert entstand, `value moved here` und die Leihe danach. Die Zeilennummer vor der dritten Marke ist die, die du gegen deine hältst, und der Fehlercode steht in eckigen Klammern in der allerersten Zeile." } ] }
...
Sage das Ergebnis des Snippets vorher, implementiere dann beide Funktionen und führe die Tests dieses Steps aus dem Block unten aus.
...
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *takes_ownership und gives_ownership bestehen*.
