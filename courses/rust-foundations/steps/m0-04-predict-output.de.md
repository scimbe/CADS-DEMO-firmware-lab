...
tasks:
  - id: guess
    ...
    check: { type: "predict", prompt: { en: "examples/m0_shadowing.rs shadows `x` three times and shadows `spaces` with a value of a different type. Write down, line by line, what the program prints - all four lines, with the exact numbers.", de: "examples/m0_shadowing.rs überschattet `x` dreimal und überschattet `spaces` mit einem Wert anderen Typs. Schreibe Zeile für Zeile auf, was das Programm ausgibt - alle vier Zeilen mit den genauen Zahlen." }, then: { type: "command", command: "cargo run --quiet --example m0_shadowing", seedMustFail: false, expectExitCode: 0, expectStdout: "inner scope is: 12", timeoutMs: 120000 }, rubric: "Die Vorhersage nennt 12 für den inneren Gültigkeitsbereich und 6 für das äußere x, 3 für spaces sowie 3 und 1 für Ganzzahldivision und Rest. Eine Vorhersage von 6 für den inneren Bereich (die zweite Überschattung übersehen) oder eines Fehlers für `spaces` (einen Typkonflikt erwartet) ist eine falsche Vorhersage, und das ist ein nützliches Ergebnis, kein Fehlschlag.", bloom: "evaluate" }
  - id: convert
...
socratic:
  - { trigger: "task:guess:failed", question: { en: "Your prediction differs from the run. Which of the four lines did you expect otherwise - one of the two x lines, or the one for spaces?", de: "Deine Vorhersage weicht vom Lauf ab. Welche der vier Zeilen hattest du anders erwartet - eine der beiden x-Zeilen oder die zu spaces?" }, hints: [ { en: "Shadowing is not assignment: every `let x = ...` makes a new variable and leaves the old one lying untouched underneath. Count how many `let x` stand before the inner print, and how many of them are still in force once the inner block has closed.", de: "Überschatten ist kein Zuweisen: jedes `let x = ...` erzeugt eine neue Variable und lässt die alte unberührt darunter liegen. Zähle, wie viele `let x` vor der inneren Ausgabe stehen, und wie viele davon noch gelten, wenn der innere Block geschlossen ist." }, { en: "Open `examples/m0_shadowing.rs` and mark the braces of the inner block. Everything inside them that begins with `let` ends at the closing brace, and the outer print never sees it.", de: "Öffne `examples/m0_shadowing.rs` und markiere die geschweiften Klammern des inneren Blocks. Alles darin, was mit `let` beginnt, endet an der schließenden Klammer, und die äußere Ausgabe sieht davon nichts." }, { en: "Work the chain out by hand from the first binding down to the inner print, writing the current value beside each `let`. Ask the question about `spaces` separately: a shadowing may change the type, an assignment may not, so decide first which of the two the second `spaces` line is.", de: "Rechne die Kette von der ersten Bindung bis zur inneren Ausgabe von Hand durch und schreibe nach jedem `let` den aktuellen Wert daneben. Die Frage zu `spaces` stelle getrennt: eine Überschattung darf den Typ wechseln, eine Zuweisung nicht - entscheide also zuerst, welches von beiden die zweite `spaces`-Zeile ist." } ] }
...
`examples/m0_shadowing.rs` liegt unter `examples/`, einem Verzeichnis, das cargo als eigenständige Programme übersetzt. Ausgeführt wird eines mit `cargo run --example <name>`. Führe dieses hier noch nicht aus: die Prüfung tut es für dich, und erst, wenn deine Vorhersage geschrieben ist.
...
::: do command="cargo test --test m0-04-predict-output" cwd="."
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *Die Temperaturumrechnungen bestehen*.
