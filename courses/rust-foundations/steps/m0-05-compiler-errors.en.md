...
- **The spans.** The carets `^^^^^` mark one place and a second span with dashes marks another, labelled `expected due to this`. Work out which of the two you have to change before you change anything - that is the question this step asks you to answer.
...
Fix **one** error, then recompile, and watch how many are left. Chasing all three at once usually produces a file with three unrelated changes, two of which were unnecessary.
