...
tasks:
  ...
  - id: read-the-test
    ...
    check: { type: "question", prompt: { en: "The test asserts greet(\"\") == \"Hello, !\". Two sentences: what that forbids in your implementation, and what a special case for the empty name would return instead.", de: "Der Test verlangt greet(\"\") == \"Hello, !\". Zwei Sätze: was das in deiner Implementierung verbietet, und was ein Sonderfall für den leeren Namen stattdessen liefern würde." }, rubric: "Erster Satz: ein Zweig auf Leerheit ist verboten, der Name wird bedingungslos eingesetzt. Zweiter Satz: ein Sonderfall lieferte etwas anderes als `Hello, !` - ein bloßes `Hello!` oder einen Platzhalternamen - und ließe diese Zusicherung scheitern. Besteht nicht: beschreiben, was format! tut, ohne zu sagen, dass der Zweig verboten ist; oder nur einen der beiden Sätze beantworten.", recallPrompt: { en: "A test asserts that greeting an empty name still produces the greeting with an empty slot. What does that forbid in the implementation, and what would a special case return instead?", de: "Ein Test verlangt, dass ein leerer Name dieselbe Begrüßung mit leerer Stelle ergibt. Was verbietet das in der Implementierung, und was lieferte ein Sonderfall stattdessen?" }, bloom: "understand", minChars: 40 }
socratic:
  - { trigger: "task:read-the-test:failed", question: { en: "Write down what your greet returns for the empty name. Does the test accept that value?", de: "Schreibe auf, was dein greet für den leeren Namen liefert. Akzeptiert der Test diesen Wert?" }, hints: [ { en: "Compare the three assertions with each other: two pass a name, one passes none, and all three demand the same shape.", de: "Vergleiche die drei Zusicherungen miteinander: zwei übergeben einen Namen, eine keinen, und alle drei verlangen dieselbe Form." }, { en: "Ask what an `if name.is_empty()` branch would have to return, then look for that string in the test.", de: "Frage, was ein Zweig `if name.is_empty()` liefern müsste, und suche diese Zeichenkette dann im Test." }, { en: "The comma and the space before the exclamation mark are part of the expected value even with an empty name - that is what the third assertion exists to pin down.", de: "Komma und Leerzeichen vor dem Ausrufezeichen gehören auch bei leerem Namen zum erwarteten Wert - genau das nagelt die dritte Zusicherung fest." } ] }
...
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *greet() liefert die Begrüßung*.
...
> recover: Antwortet cargo mit `error: could not find \`Cargo.toml\` in \`/home/coder/workspace\` or any parent directory`, hat dieses Terminal das `cd` von oben nicht bekommen - hole es nach. Meldet es `no test target named`, stimmt der Name hinter `--test` nicht; `ls tests/` listet die gültigen Namen auf.
:::
