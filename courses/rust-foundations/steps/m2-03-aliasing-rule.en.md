...
socratic:
  - { trigger: "task:two-mut:failed", question: { en: "This check wants rustc to reject the snippet, and with one particular message. Did no error come at all, or a different one?", de: "Diese Prüfung will, dass rustc den Schnipsel ablehnt, und zwar mit einer ganz bestimmten Meldung. Kam gar kein Fehler, oder ein anderer als der erwartete?" }, hints: [ { en: "Nothing here is yours to fix. Ask instead what the check is evidence of: a rule you have only read is a claim, a rule you have watched the compiler enforce is an observation. Which of the two went wrong - the observation, or the way to it?", de: "Hier ist nichts zu reparieren. Frage stattdessen, wofür die Prüfung der Beleg ist: eine Regel, die man nur gelesen hat, ist eine Behauptung, eine Regel, bei deren Durchsetzung man dem Compiler zugesehen hat, ist eine Beobachtung. Welches von beiden ist schiefgegangen - die Beobachtung oder der Weg dorthin?" }, { en: "The snippet is `snippets/m2_03_two_mut_borrows.rs`, and the command compiles it on its own, without cargo. Run it from the crate folder and read the first line of the output: the error code stands there in square brackets, and a complaint about a missing file is a different problem from a complaint about a borrow.", de: "Der Schnipsel ist `snippets/m2_03_two_mut_borrows.rs`, und der Befehl übersetzt ihn allein, ohne cargo. Führe ihn im Ordner der Crate aus und lies die erste Zeile der Ausgabe: dort steht der Fehlercode in eckigen Klammern, und eine Klage über eine fehlende Datei ist ein anderes Problem als eine Klage über eine Leihe." }, { en: "If no error came at all, the snippet compiles, which means the second `&mut` line is no longer in it. Files under `snippets/` are the observation, not the exercise: they belong unchanged, and the practising happens in `src/`. Put the second borrow back and both references have to be live at the same moment - which is exactly what the rule forbids.", de: "Kam gar kein Fehler, dann kompiliert der Schnipsel, und das heißt, die zweite `&mut`-Zeile steht nicht mehr darin. Dateien unter `snippets/` sind die Beobachtung, nicht die Übung: sie gehören unverändert, geübt wird in `src/`. Setze die zweite Leihe zurück, dann müssen beide Referenzen im selben Moment leben - und genau das verbietet die Regel." } ] }
