...
tasks:
  ...
  - id: message
    ...
    check: { type: "question", prompt: { en: "parse_port is specified to report \"'http' is not a valid port\" rather than passing on the parser's own \"invalid digit found in string\". Argue for that choice from the point of view of whoever reads the message, and name the cost of discarding the original error.", de: "parse_port soll \"'http' is not a valid port\" melden statt die eigene Meldung des Parsers \"invalid digit found in string\" durchzureichen. Begründe diese Entscheidung aus Sicht dessen, der die Meldung liest, und nenne den Preis dafür, den ursprünglichen Fehler zu verwerfen." }, rubric: "Argumentiert, dass die Meldung die störende Eingabe und den fachlichen Begriff nennen soll - einen Port -, was die allgemeine Meldung des Parsers nicht kann, und dass genau das den Benutzer seine Konfiguration reparieren lässt. Nennt den Preis ehrlich: der ursprüngliche Fehler wird verworfen, der Aufrufer kann `keine Zahl` also nicht mehr von `außerhalb des Bereichs für u16` unterscheiden und nicht programmatisch auf den Fehler verzweigen - was ein eigener Fehlertyp in m5-04 wiederherstellt. Besteht nicht: die Meldung loben, ohne zu nennen, was der verworfene Fehler dem Aufrufer unmöglich macht.", recallPrompt: { en: "A parse error is replaced by a message naming the offending input and the domain concept. Argue that from the side of whoever reads it, and name the price of discarding the original error.", de: "Ein Parse-Fehler wird durch eine Meldung ersetzt, die die störende Eingabe und den Fachbegriff nennt. Begründe das aus Sicht dessen, der sie liest, und nenne den Preis dafür, den ursprünglichen Fehler zu verwerfen." }, bloom: "evaluate", minChars: 70 }
socratic:
...
::: do command="cargo test --test m5-02-result" cwd="."
Führe die Tests dieses Steps aus. Derselbe Befehl steckt hinter dem Knopf **Prüfen** an der Aufgabe *Die vier Result-Funktionen bestehen*.
