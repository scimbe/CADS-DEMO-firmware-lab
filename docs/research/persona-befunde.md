...
| **Offene Frage, entscheidbar mit einer Beobachtung** | Hielt der Kern kurz wirklich an, oder ist das Ereignis frei erfunden? Ein angehaltener Kern kann nicht antworten, ein laufender schon — also: unmittelbar nach `Shift`+`F5`, während die Leiste „angehalten" zeigt, über die Bridge-Konsole (`/home/coder/board-console`) eine harmlose Abfrage schicken. Antwortet das Board, ist PB-02 durchgehend ein Anzeigefehler; bleibt es bei funktionierender Konsole stumm, steckt ein zweiter Fehler in der Sonde. Der Weg über die Bridge vermeidet den zweiten ST-Link-Zugriff und damit die Kontention. |
| **Warum nicht per Kamera** | Der optische Weg wurde versucht und abgebrochen: Die Bench-Kamera ist nicht erreichbar, der hartkodierte Vorgabetyp taucht in der Geräteliste nicht auf, und ein Rateversuch hätte ein Foto des Raums erzeugt. Richtig abgebrochen — eine unbeantwortete Frage ist besser als eine geratene Kamera. Drei Reproduktionen zeigen übereinstimmend `probe.core: running` binnen 1-2 s nach dem Trennen, jeweils vor jedem Klick. |
| **Stand** | Diagnose steht, Fix (a) ist unabhängig von der offenen Frage und sollte zuerst kommen. |
