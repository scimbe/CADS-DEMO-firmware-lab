...
| **Nachtrag - jetzt bestaetigt** | Naechster Schritt real ausgefuehrt, in einem frischen Wegwerf-Container: m0-01 bis m0-05 ueber die echte Bedienoberflaeche abgeschlossen (`.run-check` fuer command-/testSuite-Aufgaben, echte LLM-Bewertung fuer die Fragen, echte `predict`-Abgabe), nicht ueber synthetischen Sitzungszustand. Nach m0-05 zeigt `#step-title` **"Scope, owner, move"** - gegen die Quelldatei geprueft, nicht angenommen. Der Abrufbereich (`#recall-area`) ist gefuellt, und zwar mit dem `recallPrompt.en` von m0-03s `read-the-test` woertlich, nicht mit erfundenem Text. Damit ist die Kette einmal durchgaengig im laufenden Panel gesehen worden, nicht nur simuliert. Wegwerf-Container danach abgebaut. |
...
#### PB-19, Nachtrag: der Tutor hat nichts verraten - der Step druckt die Loesung selbst

| | |
|---|---|
| **Nachgesehen statt geglaubt** | Die uebergebene Antwort ("return the exact string `Hello, JavaScript!`") steht **woertlich im Steptext selbst**, `m0-02-first-run.en.md:103-105`, als vollstaendiger Codeblock zwei Absaetze ueber der Aufgabe. Der Step traegt `scaffold: worked` - das Vorzeigen ist Absicht, es ist die erste Uebung des Kurses. |
| **Damit ist der Befund ein anderer** | Der Tutor hat **nicht geleakt**. Er hat aus der gegruendeten Quelle geantwortet, und die Quelle ist der Kurstext, der die Loesung bewusst zeigt. Wer ihn nicht fragt, liest dieselbe Zeile eine Bildschirmhoehe weiter oben. Ein „Erklaerbetrieb verschenkt die Loesung"-Fund waere hier falsch zugeordnet. |
| **Der echte Befund** | **Ein `worked`-Step vergibt `checkFirstTry` - den einzigen starken Beleg ausser dem Abruf - fuer das Abtippen einer Loesung, die er selbst gedruckt hat.** Das ist kein Tutor-Problem und kein Mogelweg, sondern eine Luecke im Belegmodell: Abschreiben und Koennen bekommen dasselbe Gewicht. |
| **Groesse, gemessen** | 116 Steps: 36 `worked`, 43 `faded`, 37 `independent`. Von 113 ausfuehrbaren Pruefungen sitzen **35 auf einem `worked`-Step** - knapp ein Drittel. Darunter `javascript m0-02-first-run/greet`, der gepruefte Fall. |
